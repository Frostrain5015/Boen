...
const getNodeByTitleSubject = (title: string, subject: string) =>
  db.prepare(`SELECT id FROM kg_nodes WHERE type='knowledge_point' AND subject=? AND title=?`).get(subject, title) as { id: number } | undefined;
...
function seedBelongsToEdges(subject: string): number {
  let count = 0;
  for (const [kpTitle, themeCode] of Object.entries(kpThemes)) {
    ...
    if (kp && theme) { insEdge.run(kp.id, theme.id, 'belongs_to', 1.0); count++; }
  }
  return count;
}
...
function seedReinforcesEdges(subject: string): number {
  let count = 0;
  for (const [kpTitle, litCodes] of Object.entries(kpLiteracies)) {
    ...
    for (const litCode of litCodes) {
      ...
      if (lit) { insEdge.run(kp.id, lit.id, 'reinforces', 1.0); count++; }
    }
  }
  return count;
}
...
const MATH_PREREQUISITES: { from: string; to: string; subject: string }[] = [
  // ══════════════════════════════════════════════
  // G7 上册：有理数链
  // ══════════════════════════════════════════════
 ...
 * 返回成功插入的边数。
 */
export function seedPrerequisiteEdges(subject: string): number {
  let count = 0;
...
function seedBloomEdges(subject: string): number {
  let count = 0;
  for (const [kpTitle, bloomCode] of Object.entries(kpBloom)) {
    const kp = getNode('knowledge_point', undefined, kpTitle);
    const bloom = getNode('bloom_level', bloomCode);
    if (kp && bloom) { insEdge.run(kp.id, bloom.id, 'bloom_at', 1.0); count++; }
  }
  return count;
}
