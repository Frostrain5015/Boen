...
// ── 查询函数 ─────────────────────────────────
/** 根据年级返回对应的试卷结构 */
export function getExamStructure(grade) {
    const g = String(grade);
    if (['1', '2', '3', '4', '5', '6'].includes(g))
        return PRIMARY;
    if (['7', '8', '9'].includes(g))
        return MIDDLE;
    return HIGH;
...
/** 根据年级段标识获取结构 */
export function getExamStructureByBand(band) {
    return STRUCTURES[band];
}
/** 根据总分推荐最近似的年级段结构 */
export function getStructureByTotalScore(totalScore) {
    if (totalScore <= 60)
        return PRIMARY;
    if (totalScore <= 120)
        return MIDDLE;
 ...
 */
export function getQuestionTypesForMode(grade, mode, targetScore) {
