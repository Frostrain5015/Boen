...
import { cacheProficiencyUpdate, getCachedProficiencySum, getCachedProficiencyExpected, setCachedProficiencyExpected, hasCachedExpectedRating, flushProficiencyCache, computeProficiencyDelta, } from '../knowledge-profile.js';
...
describe('hasCachedExpectedRating / getCachedProficiencyExpected / setCachedProficiencyExpected', () => {
    ...
    it('getCachedProficiencyExpected 首次应初始化并返回 dbRating', () => {
        cacheProficiencyUpdate(USER, THREAD, KP_A, 1, 1, 'review');
        const { rating, sigma, lastUpdated } = getCachedProficiencyExpected(USER, THREAD, KP_A, 0, 20, 0);
    ...
    it('逐题增量：连续三次计算，每次 oldRating 应等于上次 newRating', () => {
        cacheProficiencyUpdate(USER, THREAD, KP_A, 0, 1, 'review');
        // 第 1 题：错
        let { rating: oldR, sigma: oldS } = getCachedProficiencyExpected(USER, THREAD, KP_A, 0, 20, 0);
