...
import { computeProficiencyDelta, cacheProficiencyUpdate, getCachedProficiencyExpected, setCachedProficiencyExpected, hasCachedExpectedRating, flushProficiencyCache, difficultyLevelToValue, ELO_RATING_INIT, ELO_SIGMA_INIT, } from '../knowledge-profile.js';
// ── 配置 ───────────────────────────────────────
        ...
        it(`${mode} 模式 ${ITERATIONS} 次随机序列`, () => {
            for (let seq = 0; seq < ITERATIONS; seq++) {
                ...
                for (let q = 0; q < qCount; q++) {
                    ...
                    const result = computeProficiencyDelta(rating, sigma, actualScore, maxScore, mode, 0, diffVal);
                    // 不变量断言
                    expect(result.newRating).withContext(`mode=${mode} seq=${seq} q=${q}: rating out of range`).toBeGreaterThanOrEqual(0);
                    expect(result.newRating).toBeLessThanOrEqual(100);
                    expect(result.newSigma).withContext(`sigma out of range: ${result.newSigma}`).toBeGreaterThanOrEqual(3);
    ...
    for (const mode of MODES) {
        it(`${mode} 模式 ${ITERATIONS} 次缓存操作`, () => {
            ...
            for (let round = 0; round < ITERATIONS; round++) {
                ...
                const expected = getCachedProficiencyExpected(userId, threadId, kp, ratings[kp] ?? ELO_RATING_INIT, sigmas[kp] ?? ELO_SIGMA_INIT, 0);
...
describe('压力测试：长会话疲劳', () => {
    it('100 题连续答题不崩溃', () => {
        ...
        for (let q = 0; q < 100; q++) {
            ...
            const result = computeProficiencyDelta(rating, sigma, randomCorrect(), 1, MODE, 0, diff);
            rating = result.newRating;
            sigma = result.newSigma;
        }
