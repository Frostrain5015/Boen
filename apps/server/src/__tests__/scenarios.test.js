    ...
    for (const sc of SCENARIOS) {
        it(sc.name, () => {
            ...
            for (const step of steps) {
                const result = computeProficiencyDelta(rating, sigma, step.score, step.maxScore, sc.mode, 0);
                rating = result.newRating;
                sigma = result.newSigma;
            }
            const stars = starVal(rating);
            expect(stars).withContext(`mode=${sc.mode}, steps=${sc.steps}: rating=${Math.round(rating)}, stars=${stars}`).toBeGreaterThanOrEqual(sc.expectMinStars);
        ...
        for (const pattern of PATTERNS) {
            it(`${mode} ${pattern}`, () => {
                ...
                for (const ch of pattern) {
                    const result = computeProficiencyDelta(rating, sigma, ch === '1' ? 1 : 0, 1, mode, 0);
                    rating = result.newRating;
                    sigma = result.newSigma;
                }
    ...
    for (const sc of SCENARIOS) {
        it(sc.name, () => {
            ...
            for (let i = 0; i < sc.steps.length; i++) {
                ...
                const result = computeProficiencyDelta(rating, sigma, ch === '1' ? 1 : 0, 1, 'review', 0, diff);
                rating = result.newRating;
                sigma = result.newSigma;
            }
