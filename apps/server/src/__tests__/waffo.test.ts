...
let remote: { id: string; status: string; isInTrial: boolean; testMode: boolean; merchantProvidedBuyerIdentity: string; currentPeriodStart: string; currentPeriodEnd: string; canceledAt: string | null; updatedAt: string; subscriptionProduct: { id: string } };
...
beforeEach(() => {
  vi.clearAllMocks();
  Object.assign(process.env, { WAFFO_MERCHANT_ID: 'merchant', WAFFO_PRIVATE_KEY: 'key', WAFFO_STORE_ID: 'store', WAFFO_PRODUCT_MONTHLY: 'product', WAFFO_ENVIRONMENT: 'test', WAFFO_ALLOW_TEST_CHECKOUT: 'true' });
  ...
  db.exec(`CREATE TABLE subscriptions(user_id TEXT PRIMARY KEY,tier TEXT,expires_at INTEGER,activated_at INTEGER);
    CREATE TABLE payment_orders(session_id TEXT PRIMARY KEY,user_id TEXT,plan_key TEXT,product_id TEXT,currency TEXT,external_id TEXT UNIQUE,order_id TEXT,checkout_url TEXT,buyer_email TEXT,status TEXT,amount TEXT,created_at INTEGER DEFAULT(unixepoch()),updated_at INTEGER);
  ...
  remote = { id: 'order', status: 'active', isInTrial: true, testMode: true, merchantProvidedBuyerIdentity: 'alice', currentPeriodStart: iso(now), currentPeriodEnd: iso(now + 7 * 86400), canceledAt: null, updatedAt: iso(now), subscriptionProduct: { id: 'product' } };
  mocks.query.mockImplementation(async () => ({ data: { subscriptionOrder: remote, subscriptionOrders: [] } }));
