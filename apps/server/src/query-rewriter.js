...
function shouldSkipRewrite(query) {
    ...
    if (t.length < 6)
        return true;
    // 问候
    if (/^(你好|嗨|hi|hello|hey|早|晚上好|谢谢|感谢)/i.test(t))
        return true;
    // 简短确认/追问
    if (/^(好[的吧么]?|行[吧]?|可以|嗯嗯|ok|是的|对[的啊]?|没错|继续|明白|知道了?|为什么|然后[呢]?|所以[呢]?|再讲讲|还有[吗]?)/i.test(t))
        return true;
    return false;
}
let _model = null;
 ...
 */
export function setRewriteModel(model) {
    _model = model;
}
/**
 * 将学生提问改写为更适合检索的查询。
 ...
 */
export async function rewriteQuery(originalQuery, subject, grade) {
    if (shouldSkipRewrite(originalQuery) || !_model)
    ...
    try {
        ...
        // 防止模型返回空或与原文完全无关的内容
        if (!rewritten || rewritten.length > originalQuery.length * 3)
            return originalQuery;
        return rewritten;
    }
    catch {
