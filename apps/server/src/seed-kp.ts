...
function gradeLabel(g: string): string {
  const n = Number(g);
  if (n >= 1 && n <= 9) return n <= 6 ? `小学${GRADE_CN[n - 1]}年级` : `初中${GRADE_CN[n - 1]}`;
  return g;
}
...
/** 从 DB 读取某教材的完整章节树 */
function loadUnitTree(textbookId: number, parentId: number | null = null): UnitNode[] {
...
/** 扁平化章节树为标题列表 */
function flattenTree(nodes: UnitNode[], depth = 0): string[] {
  const lines: string[] = [];
  for (const n of nodes) {
    lines.push('  '.repeat(depth) + '- ' + n.title);
    if (n.children.length) lines.push(...flattenTree(n.children, depth + 1));
  }
  return lines;
}

/** 检查某教材是否已有知识点 */
function hasKnowledgePoints(textbookId: number): boolean {
  const count = db.prepare(`
    SELECT COUNT(*) AS c FROM unit_knowledge_map ukm
    JOIN curriculum_units u ON u.id = ukm.unit_id
    WHERE u.textbook_id=?
  `).get(textbookId) as { c: number };
  return count.c > 0;
}
...
async function main() {
  ...
  const textbooks = db.prepare(
    `SELECT id, subject, grade, volume FROM curriculum_textbooks ORDER BY subject, grade, volume`
  ).all() as { id: number; subject: string; grade: string; volume: string }[];
  ...
  for (const tb of textbooks) {
    ...
    for (const kp of parsed) {
      ...
      if (kpId === 0) {
        // 已存在，查找 id
        const existing = db.prepare(`SELECT id FROM knowledge_points WHERE subject=? AND title=?`).get(tb.subject, kp.title) as { id: number } | undefined;
      ...
      for (const title of titles) {
        ...
        const matches = db.prepare(
          `SELECT id FROM curriculum_units WHERE textbook_id=? AND (title=? OR title LIKE ?) ORDER BY seq LIMIT 1`
        ).all(tb.id, title, `%${title.replace(/[%_]/g, '')}%`) as { id: number }[];
    ...
    if (mapped === 0) {
      ...
      for (const kp of parsed) {
        const kpRow = db.prepare(`SELECT id FROM knowledge_points WHERE subject=? AND title=?`).get(tb.subject, kp.title) as { id: number } | undefined;
        if (!kpRow) continue;
        // 关联到第一个匹配的父级章节
        const allUnits = db.prepare(
          `SELECT id, title FROM curriculum_units WHERE textbook_id=? ORDER BY seq`
        ).all(tb.id) as { id: number; title: string }[];
