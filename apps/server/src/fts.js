 ...
 */
export function indexFtsRow(refType, refId, subject, grade, content) {
    db.prepare(`
...
export function rebuildFtsIndex() {
    ...
    const count = db.prepare(`SELECT COUNT(*) as cnt FROM curriculum_fts`).get();
    if (count && count.cnt > 0)
        return count.cnt;
    // 清空旧索引（幂等安全）
    db.exec(`DELETE FROM curriculum_fts`);
    const rows = db.prepare(`SELECT ref_type, ref_id, subject, grade, content FROM curriculum_embeddings`).all();
 ...
 */
export function searchFts(subject, grade, query, limit = 10) {
    ...
    try {
      ...
      ORDER BY bm25_score
      LIMIT ?
    `).all(subject, grade, sanitized, limit);
