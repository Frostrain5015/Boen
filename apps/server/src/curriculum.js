...
export async function ingestTextbook(seed) {
    ...
    const existing = db.prepare(`SELECT id FROM curriculum_textbooks WHERE subject=? AND grade=? AND volume=? AND publisher=?`).get(subject, grade, volume, publisher);
    if (existing) {
        const oldUnitIds = db.prepare(`SELECT id FROM curriculum_units WHERE textbook_id=?`).all(existing.id);
    ...
    // 2) 教材
    const tbId = Number(db.prepare(`INSERT INTO curriculum_textbooks (subject, grade, volume, publisher, version, source_url) VALUES (?,?,?,?,?,?)`).run(subject, grade, volume, publisher, seed.version ?? null, seed.sourceUrl ?? null).lastInsertRowid);
    ...
    for (const kp of seed.knowledgePoints ?? []) {
        const hit = findKp.get(subject, kp.title);
    ...
    const unitRows = [];
    function walk(nodes, parentId, trail) {
        nodes.forEach((node, i) => {
            ...
            if (node.children?.length)
                walk(node.children, uid, crumb);
        });
    }
    walk(seed.units, null, []);
    // 5) 批量计算 embedding 并存（unit 用 breadcrumb，kp 用 标题：描述）
    const insEmb = db.prepare(`INSERT OR REPLACE INTO curriculum_embeddings (ref_type, ref_id, subject, grade, content, embedding) VALUES (?,?,?,?,?,?)`);
...
export function getOutline(grade, subject) {
    const books = db.prepare(`SELECT id, volume FROM curriculum_textbooks WHERE grade=? AND subject=? ORDER BY volume`).all(grade, subject);
    if (books.length === 0)
        return '';
    const lines = [];
    const childrenOf = (textbookId, parentId) => db.prepare(`SELECT id, title FROM curriculum_units WHERE textbook_id=? AND parent_id IS ? ORDER BY seq`).all(textbookId, parentId);
...
export async function retrieveRelated(grade, subject, query, k = 5) {
    const rows = db.prepare(`SELECT content, embedding FROM curriculum_embeddings WHERE grade=? AND subject=?`).all(grade, subject);
    if (rows.length === 0)
        return [];
 ...
 * 提供比纯向量检索更鲁棒的召回结果。
 */
async function hybridRetrieve(grade, subject, query, k = 5) {
...
export async function lookupKnowledgePoint(args) {
    ...
    if (!grade || !subject || !/^[1-9]$/.test(grade))
        return '当前年级或学科不在课程知识库范围内。';
    if (!query)
        return '查询词为空，无法检索课程知识点。';
    const parts = [];
    // mode: keyword / auto(含精确匹配倾向) → 执行 FTS5 关键词搜索 + LIKE 查找
    if (mode === 'keyword' || mode === 'hybrid' || mode === 'auto') {
        const like = `%${escapeLike(query)}%`;
        const matches = db.prepare(`SELECT title, description, code
       FROM knowledge_points
       WHERE subject=?
         AND (grade=? OR grade IS NULL)
         AND (title LIKE ? ESCAPE '\\' OR description LIKE ? ESCAPE '\\' OR code LIKE ? ESCAPE '\\')
       ORDER BY title
       LIMIT ?`).all(subject, grade, like, like, like, limit);
        if (matches.length) {
            parts.push('标题/描述匹配的知识点：\n' +
                matches
    ...
    if (parts.length === 0)
        return `没有在${gradeCn(grade)}·${SUBJECT_LABEL[subject] ?? subject}课程知识库中找到「${query}」。`;
    return parts.join('\n\n');
}
/**
 ...
 */
export async function retrieveCurriculum(args) {
    ...
    const kpList = db.prepare(`SELECT n.id, n.title FROM kg_nodes n
     ...
     GROUP BY n.id ORDER BY n.title`).all(subject, grade);
    if (kpList.length > 0) {
        ...
        if (kpList.length > 50)
            parts.push(`- ……（共 ${kpList.length} 个，仅列出前 50）`);
    }
    return parts.join('\n');
}
