...
import { getExamStructure } from './exam-structures.js';
...
function sortQuestionTypes(items) {
    return [...items].sort((a, b) => (QUESTION_TYPE_ORDER[a.type] ?? 99) - (QUESTION_TYPE_ORDER[b.type] ?? 99));
}
function tuneCountsToTarget(items, targetScore) {
    if (!items.length)
        return 0;
    let dp = new Map([[0, { cost: 0, counts: [] }]]);
...
function finalizeBlueprint(bp, targetScore) {
 ...
 */
export function buildConstraintBoundary(grade, mode, totalScore) {
    const struct = getExamStructure(grade);
    const variant = mode === 'quiz' ? struct.quiz : struct.exam;
    const typeGuidance = sortQuestionTypes(variant.questionTypes).map(qt => `  - ${qt.label}（${qt.type}）：标准 ${qt.count} 题 × ${qt.pointsPer} 分/题`).join('\n');
    return [
...
// ── 降级蓝图（LLM 失败时的兜底） ──────────────────────────
export function defaultBlueprint(config, mode = 'exam') {
    ...
    const questionTypes = sortQuestionTypes(variant.questionTypes).map(qt => ({
        ...qt,
        count: Math.max(1, Math.round(qt.count * ratio)),
    }));
    // 精确修正舍入误差：暴力遍历所有题型 count 组合，找到总分等于 target 的配置
    const exactScore = (qts) => qts.reduce((s, qt) => s + qt.count * qt.pointsPer, 0);
...
// ── 蓝图架构师主函数 ──────────────────────────────────────
export async function stepBlueprintArchitect(model, config, weightGuide, profileContext, mode) {
    const totalScore = config.totalScore ?? 100;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            const response = await model.invoke([new SystemMessage(prompt + '\n\n必须直接输出纯净 JSON 格式的蓝图，不要 markdown 代码块，不要其他文字。')], { response_format: { type: 'json_object' } });
            const content = typeof response.content === 'string' ? response.content : '';
            const parsed = JSON.parse(content);
            const validated = examBlueprintSchema.safeParse(parsed);
            if (validated.success)
                return validateAndFixBlueprint(validated.data, totalScore);
            // schema 不匹配：尝试修复
...
function validateAndFixBlueprint(bp, targetScore) {
    const allQts = [];
    ...
    let newScore = fixedSections.reduce((s, sec) => s + sec.questionTypes.reduce((ss, qt) => ss + qt.count * qt.pointsPer, 0), 0);
    ...
    if (Math.abs(newDiff) / targetScore < 0.10 && newDiff !== 0) {
        ...
        for (const qt of allQts) {
            for (let delta = 1; delta <= 5; delta++) {
                ...
                if (Math.abs(adjustment * qt.pointsPer) > Math.abs(newDiff))
                    break;
                const newC = qt.count + adjustment;
                if (newC >= 1 && newC <= 20) {
                    const trial = qt.count + adjustment;
                    const trialScore = fixedSections.reduce((s, sec) => s + sec.questionTypes.reduce((ss, q) => ss + q.count * q.pointsPer, 0), 0) -
...
/** 尝试修复常见的 schema 校验失败 */
function tryFixBlueprint(raw, targetScore) {
    try {
        // 补全根级缺失字段
        if (!raw.title)
            raw.title = '综合试卷';
        if (!raw.totalScore)
            raw.totalScore = targetScore;
        // 补全 / 修复 sections
        if (Array.isArray(raw.sections)) {
            for (const sec of raw.sections) {
                ...
                if (Array.isArray(sec.knowledgePoints)) {
                    sec.knowledgePoints = sec.knowledgePoints.map((kp) => typeof kp === 'string' ? { title: kp, weight: 0.5 } : kp);
                }
        ...
        // 补全 coveragePlan
        if (!raw.coveragePlan) {
            const allKps = (raw.sections ?? []).flatMap((s) => (s.knowledgePoints ?? []).map((kp) => kp.title ?? kp));
...
export function flattenBlueprint(blueprint) {
    ...
    return tasks.sort((a, b) => (QUESTION_TYPE_ORDER[a.questionType] ?? 99) - (QUESTION_TYPE_ORDER[b.questionType] ?? 99) ||
        (a.sectionOrder ?? 0) - (b.sectionOrder ?? 0)).map(({ sectionOrder: _sectionOrder, ...task }) => task);
}
