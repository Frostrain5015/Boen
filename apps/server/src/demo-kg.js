...
const tb = db.prepare(`SELECT * FROM curriculum_textbooks WHERE subject='math' AND grade='7' AND volume='上册'`).get();
console.log('教材:', tb.version);
console.log('来源:', tb.source_url);
console.log('');
const chapters = db.prepare(`SELECT * FROM curriculum_units WHERE textbook_id=? AND parent_id IS NULL ORDER BY seq`).all(tb.id);
for (const ch of chapters) {
    console.log('■', ch.title);
    const sections = db.prepare(`SELECT * FROM curriculum_units WHERE parent_id=? ORDER BY seq`).all(ch.id);
    for (const sec of sections) {
      ...
      WHERE ukm.unit_id=?
    `).all(sec.id);
        const kpStr = kps.length ? '  → [' + kps.map((k) => k.title).join(', ') + ']' : '';
        const subCount = db.prepare(`SELECT COUNT(*) AS n FROM curriculum_units WHERE parent_id=?`).get(sec.id).n;
        const hasSub = subCount > 0;
        console.log('  ├', sec.title, hasSub ? `(${subCount} 子节)` : '', kpStr);
        if (hasSub) {
            const subs = db.prepare(`SELECT * FROM curriculum_units WHERE parent_id=? ORDER BY seq`).all(sec.id);
          ...
          WHERE ukm.unit_id=?
        `).all(sub.id);
                const subkpStr = subkps.length ? '  → [' + subkps.map((k) => k.title).join(', ') + ']' : '';
...
const firstUnit = db.prepare(`SELECT id FROM curriculum_units WHERE parent_id=? ORDER BY seq LIMIT 1`).get(firstCh.id);
if (firstUnit) {
