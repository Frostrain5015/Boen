...
function packPayload(p) {
    ...
    const term = ((p.version & MAX_VERSION) << 13) |
...
function fromCrockford(input, outLen) {
...
function groupHyphens(raw) {
    return raw.replace(/(.{4})/g, '$1-').replace(/-$/, '');
}
export function encodeCode(p, secret) {
...
export function decodeCode(code, secret) {
...
// ── 生成（CLI 用）─────────────────────────────────────────────
export function generateCode(opts, secret) {
 ...
 */
export function grantMembershipDays(userId, days) {
    const now = Math.floor(Date.now() / 1000);
    const subRow = db.prepare(`SELECT expires_at FROM subscriptions WHERE user_id=?`).get(userId);
    const base = subRow?.expires_at && subRow.expires_at > now ? subRow.expires_at : now;
    const until = base + days * 86400;
    const tier = days >= 365 ? 'yearly' : 'monthly';
    db.prepare(`
 ...
 */
export function redeemForUser(userId, rawCode, secret) {
    const p = decodeCode(rawCode, secret);
    if (!p || p.version !== VERSION)
        return { ok: false, error: 'invalid_code' };
    // 撤销名单：单码 nonce 或整批 batch
    const revoked = db
        .prepare(`SELECT 1 FROM code_revocations WHERE (scope='nonce' AND value=?) OR (scope='batch' AND value=?) LIMIT 1`)
    ...
    try {
        return db.transaction(() => {
            ...
            const used = db.prepare(`SELECT COUNT(*) AS c FROM code_redemptions WHERE nonce=?`).get(p.nonce).c;
            if (p.maxUses > 0 && used >= p.maxUses)
                return { ok: false, error: 'code_used' };
            const { until } = grantMembershipDays(userId, p.durationDays);
            db.prepare(`INSERT INTO code_redemptions (nonce, user_id, duration_days, granted_until) VALUES (?, ?, ?, ?)`).run(p.nonce, userId, p.durationDays, until);
