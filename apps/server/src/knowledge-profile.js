...
export function isProfileOutlineNoise(title) {
    const clean = title.replace(/\s+/g, '').replace(/^☆/, '');
    if (PROFILE_OUTLINE_NOISE.test(clean))
        return true;
...
export function isExploreSection(title) {
    const clean = title.replace(/\s+/g, '').replace(/^[★☆]/, '');
...
export function getProficiencyLevel(weightedScore) {
    if (weightedScore >= PROFICIENCY_THRESHOLDS.mastered)
        return 'mastered';
    if (weightedScore >= PROFICIENCY_THRESHOLDS.proficient)
        return 'proficient';
    if (weightedScore >= PROFICIENCY_THRESHOLDS.developing)
        return 'developing';
...
export const MODE_ELO_MULTIPLIERS = {
    qa: 1.0,
    preview: 0.7,
    /** 课堂复习巩固 */
    review: 2.0,
...
export function difficultyLevelToValue(level) {
    if (level === 'easy')
        return 35;
    if (level === 'hard')
        return 65;
...
export function expectedCorrectness(rating, difficulty) {
    return 1 / (1 + Math.exp(-(rating - difficulty) / ELO_SCALING));
}
/** 根据不确定度和模式计算 K-factor */
function computeKFactor(sigma, mode) {
...
/** Elo 评分更新核心 */
function updateRatingElo(oldRating, oldSigma, observed, expected, mode) {
 ...
 * Elo 计算逻辑保持一致。
 */
export function computeProficiencyDelta(oldRating, oldSigma, score, maxScore, mode, lastUpdated, difficulty = ELO_DEFAULT_DIFFICULTY) {
...
function applyForgetting(sigma, lastUpdated, now) {
    if (lastUpdated <= 0)
        return sigma;
...
function adaptiveAlpha(effectivePct, oldWeighted) {
...
const PROFICIENCY_CACHE = new Map();
const MAX_PROFICIENCY_CACHE = 5000;
/** 清理 PROFICIENCY_CACHE 超出上限的条目（删除最旧的 10%） */
function trimProficiencyCache() {
    if (PROFICIENCY_CACHE.size < MAX_PROFICIENCY_CACHE)
        return;
...
function cacheKey(userId, threadId) {
 ...
 */
export function cacheProficiencyUpdate(userId, threadId, kgNodeId, score, maxScore, mode) {
 ...
 */
export function flushProficiencyCache(userId, threadId) {
    const key = cacheKey(userId, threadId);
    const userCache = PROFICIENCY_CACHE.get(key);
    ...
    for (const [kgNodeId, update] of userCache) {
        // 读旧值（含答题计数）
        const oldRow = db.prepare('SELECT weighted_score, correct_count, total_count FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, kgNodeId);
      ...
      `).run(userId, kgNodeId, oldCorrect + update.score, oldTotal + update.maxScore, Math.round(update.expectedRating), update.expectedRating, update.expectedSigma, now);
        ...
        `).all(kgNodeId);
                for (const prereq of prereqs) {
                    const prev = db.prepare(`SELECT rating, rating_sigma, last_updated FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?`).get(userId, prereq.source_id);
                    if (!prev)
                        continue;
        ...
        // 读新值
        const newRow = db.prepare('SELECT weighted_score FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, kgNodeId);
        const after = newRow?.weighted_score ?? -1;
        // 查节点名称
        const node = db.prepare('SELECT title FROM kg_nodes WHERE id=?').get(kgNodeId);
 ...
 */
export function discardProficiencyCache(userId, threadId) {
    const key = cacheKey(userId, threadId);
    PROFICIENCY_CACHE.delete(key);
}
/**
 ...
 * 若缓存中不存在该知识点则返回 null。
 */
export function getCachedProficiencySum(userId, threadId, kgNodeId) {
    const key = cacheKey(userId, threadId);
    const userCache = PROFICIENCY_CACHE.get(key);
    if (!userCache)
        return null;
 ...
 */
export function hasCachedExpectedRating(userId, threadId, kgNodeId) {
    const key = cacheKey(userId, threadId);
    const userCache = PROFICIENCY_CACHE.get(key);
    const entry = userCache?.get(kgNodeId);
 ...
 * 而不是从会话起点累计的总额变化。
 */
export function getCachedProficiencyExpected(userId, threadId, kgNodeId, dbRating, dbSigma, dbLastUpdated) {
    const key = cacheKey(userId, threadId);
    const userCache = PROFICIENCY_CACHE.get(key);
    const entry = userCache?.get(kgNodeId);
 ...
 */
export function setCachedProficiencyExpected(userId, threadId, kgNodeId, newRating, newSigma) {
    const key = cacheKey(userId, threadId);
    const userCache = PROFICIENCY_CACHE.get(key);
    const entry = userCache?.get(kgNodeId);
    if (entry) {
...
// ── CRUD ─────────────────────────────────────
export function updateProficiency(userId, kgNodeId, score, maxScore, mode, difficulty = ELO_DEFAULT_DIFFICULTY) {
    const existing = db.prepare(`SELECT * FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?`).get(userId, kgNodeId);
...
export function getProficiency(userId, kgNodeId) {
...
export function getAllProficiencies(userId, subject, grade) {
    let sql = `
    SELECT p.*, n.title FROM user_kp_proficiency p
    JOIN kg_nodes n ON n.id = p.kg_node_id
    WHERE p.user_id=?
...
// ── 弱点 / 优势 ─────────────────────────────
export function getWeakPoints(userId, subject, grade, threshold = 60, limit = 10) {
    const all = getAllProficiencies(userId, subject, grade);
    return all.filter((p) => p.weightedScore < threshold).slice(0, limit);
}
export function getStrongPoints(userId, subject, grade, threshold = 75, limit = 10) {
    const all = getAllProficiencies(userId, subject, grade);
    return all.filter((p) => p.weightedScore >= threshold).sort((a, b) => b.weightedScore - a.weightedScore).slice(0, limit);
}
// ── 素养聚合 ───────────────────────────────
export function getLiteracyProficiency(userId, subject) {
    // 找该学科所有 literacy 节点
    const lits = db.prepare(`SELECT id, code, title FROM kg_nodes WHERE type='literacy' AND subject=?`).all(subject);
    const result = [];
    ...
    `).all(lit.id);
        ...
        for (const kp of kps) {
            const prof = db.prepare(`SELECT correct_count, total_count FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?`).get(userId, kp.id);
            if (prof) {
...
// ── 弱点链追溯 ─────────────────────────────
export function getPrerequisiteWeaknessChain(userId, kgNodeId, maxDepth = 5) {
    const chain = [];
    ...
    for (let i = 0; i < maxDepth && currentId && !visited.has(currentId); i++) {
        ...
        const node = db.prepare(`SELECT id, title FROM kg_nodes WHERE id=?`).get(currentId);
        if (!node)
            break;
        chain.push({
            nodeId: currentId,
...
// ── 推荐 ───────────────────────────────────
export function getRecommendedKPs(userId, subject, grade, limit = 5) {
...
// ── 历史回填 ───────────────────────────────
export function seedProficiencyFromHistory(userId) {
        ...
        try {
            ...
            if (meta.__boen_type === 'grading_result' && meta.result?.knowledgePoints?.length) {
                for (const kpName of meta.result.knowledgePoints) {
                    ...
                    let node = db.prepare(`SELECT id FROM kg_nodes WHERE type='knowledge_point' AND title=?`).get(kpName);
                    if (!node)
                        node = db.prepare(`SELECT id FROM kg_nodes WHERE type='knowledge_point' AND title LIKE ?`).get(`%${kpName}%`);
                    if (!node)
                        node = db.prepare(`SELECT id FROM kg_nodes WHERE type='knowledge_point' AND ? LIKE '%' || title`).get(kpName);
                    if (node) {
 ...
 * 前端画像视图的唯一数据源。
 */
export function getProfileOutline(subject, grade, userId) {
    const tbs = db.prepare(`SELECT id, volume FROM curriculum_textbooks WHERE subject=? AND grade=? ORDER BY volume`).all(subject, grade);
    const textbookNodes = [];
    let overallSum = 0, overallCount = 0;
    let weakCount = 0, goodCount = 0, masteredCount = 0;
    function enrichUnit(unitId, title) {
        // 探索型章节不参与上级聚合，不加载知识点
      ...
      `).all(kp.id);
        ...
        for (const kp of kpNodes) {
            if (kp.weightedScore >= 0) {
                ...
                overallCount++;
                if (kp.weightedScore < 60)
                    weakCount++;
                else if (kp.weightedScore >= 80)
                    masteredCount++;
                else
                    goodCount++;
            }
    ...
    function weightedAvg(kps) {
        const practiced = kps.filter((k) => k.weightedScore >= 0);
        if (!practiced.length)
            return -1;
    ...
    for (const tb of tbs) {
        const chapters = db.prepare(`SELECT id, title FROM curriculum_units WHERE textbook_id=? AND parent_id IS NULL ORDER BY seq`).all(tb.id).filter((unit) => !isProfileOutlineNoise(unit.title));
        const chapterNodes = chapters.map((ch) => {
            const sections = db.prepare(`SELECT id, title FROM curriculum_units WHERE parent_id=? ORDER BY seq`).all(ch.id).filter((unit) => !isProfileOutlineNoise(unit.title));
            const sectionNodes = sections.map((sec) => enrichUnit(sec.id, sec.title));
            const chKps = sectionNodes.flatMap((s) => s.knowledgePoints);
        ...
        // 每册的综合进度
        const tbKps = chapterNodes.flatMap((c) => c.children.flatMap((s) => s.knowledgePoints));
    ...
    const allKps = textbookNodes.flatMap((t) => t.chapters.flatMap((c) => c.children.flatMap((s) => s.knowledgePoints)));
