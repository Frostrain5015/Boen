...
import { ensureKnowledgeGraphTables, seedKnowledgeGraph, } from './knowledge-graph.js';
import { ensureWeightTable, seedWeights } from './kg-weights.js';
const SUBJECTS = ['chinese', 'english', 'math', 'science'];
function hashCode(subject, title) {
    return `kp_${subject}_${createHash('sha256').update(`${subject}:${title}`).digest('hex').slice(0, 20)}`;
}
function asSubject(value) {
    return SUBJECTS.includes(value) ? value : null;
}
...
function addPrerequisite(mapping, from, to) {
    if (typeof from !== 'string' || typeof to !== 'string')
        return;
    const a = from.trim();
    const b = to.trim();
    if (a && b && a !== b)
        mapping.prerequisites.push({ from: a, to: b });
}
function normalizeAssignment(value, output) {
    for (const [key, raw] of Object.entries(plainObject(value))) {
        ...
        if (Array.isArray(raw)) {
            for (const title of toStringArray(raw))
                output.set(title, key);
        }
    }
}
function normalizeLiteracyAssignment(value, output) {
    for (const [key, raw] of Object.entries(plainObject(value))) {
        if (!Array.isArray(raw))
            continue;
...
export function normalizeKgMapping(raw) {
    ...
    for (const rawNode of Object.values(plainObject(source.kpMap ?? source.nodes))) {
        ...
        if (typeof node.theme === 'string')
            mapping.themes.set(title, node.theme);
        const literacies = toStringArray(node.literacies);
        if (literacies.length)
            mapping.literacies.set(title, literacies);
        if (typeof node.bloom === 'string')
            mapping.blooms.set(title, node.bloom);
...
function readMappings() {
    ...
    for (const subject of SUBJECTS)
        result.set(subject, emptyMapping());
    const dir = join(CURRICULUM_DIR, 'kg-mappings');
    if (!existsSync(dir))
        return result;
    for (const file of readdirSync(dir)) {
        if (!/^[a-z]+-G\d+\.json$/.test(file))
            continue;
        try {
...
function getKnowledgeNode(subject, title) {
    return db.prepare(`SELECT id, title FROM kg_nodes WHERE type='knowledge_point' AND subject=? AND title=?`).get(subject, title);
}
function ensureEdge(sourceId, targetId, type, kind, extra = {}) {
    db.prepare(`
...
function inferTheme(subject, title) {
    const text = title.toLowerCase();
    if (subject === 'math') {
        if (/统计|数据|概率|平均|中位|众数|频数|图表|扇形|条形|方差/.test(text))
            return 'math_stats';
        if (/图形|几何|角|线|三角|四边|圆|坐标|平行|垂直|相交|面积|体积|周长|长度|位置|对称|投影|视图|立体|比例尺/.test(text))
            return 'math_geo';
        if (/实践|综合|探索|活动|建模|策略/.test(text))
            return 'math_comprehensive';
        return 'math_num_algebra';
    }
    if (subject === 'chinese') {
        if (/拼音|汉字|识字|书写|词语|词汇|成语|标点|病句|语法|朗读/.test(text))
            return 'chi_literacy';
        if (/写作|作文|习作|书信|日记|演讲|口语|表达|描写/.test(text))
            return 'chi_writing';
        if (/综合|实践|探究|文化|活动/.test(text))
            return 'chi_comprehensive';
        return 'chi_reading';
    }
    if (subject === 'english') {
        if (/听力/.test(text))
            return 'eng_listening';
        if (/口语|对话|交流|演讲|问候|介绍/.test(text))
            return 'eng_speaking';
        if (/阅读|语篇|完形|理解|检索/.test(text))
            return 'eng_reading';
        if (/写作|作文|书信|邮件|日记/.test(text))
            return 'eng_writing';
        return 'eng_language';
    }
    if (/生物|细胞|人体|植物|动物|遗传|生态|生命/.test(text))
        return 'sci_life';
    if (/地球|天体|宇宙|气候|天气|岩石|矿物|地形|灾害/.test(text))
        return 'sci_earth';
    if (/工程|技术|设计|材料|工具|控制|模型制作/.test(text))
        return 'sci_tech';
    return 'sci_matter';
}
function inferBloom(title) {
    if (/记忆|识别|名称|字母|拼音|词汇/.test(title))
        return 'bloom_remember';
    if (/概念|定义|性质|认识|组成|分类|特点|意义/.test(title))
        return 'bloom_understand';
    if (/分析|比较|判断|辨析|归纳|推理|阅读理解/.test(title))
        return 'bloom_analyze';
    if (/设计|创作|写作|制作|方案|探究报告/.test(title))
        return 'bloom_create';
    if (/评价|反思|鉴赏/.test(title))
        return 'bloom_evaluate';
    return 'bloom_apply';
}
function hasOutgoing(nodeId, type) {
 ...
 */
function dropOrphanDuplicateKnowledgeNodes() {
    ...
    const tx = db.transaction(() => {
        for (const group of groups) {
            const candidates = group.ids.split(',').map(Number).filter(Number.isInteger).map((id) => ({
                id,
                references: referenceCount.get(id, id, id, id, id, id, id).n,
  ...
  `);
    ...
    const tx = db.transaction(() => {
        for (const row of rows) {
            let node = find.get(row.subject, row.title);
            if (!node) {
...
function seedThemeLiteracyOntology() {
    for (const subject of SUBJECTS) {
        for (const [themeCode, literacyCodes] of Object.entries(THEME_LITERACIES[subject])) {
            ...
            for (const literacyCode of literacyCodes) {
                ...
                if (literacy)
                    ensureEdge(theme.id, literacy.id, 'belongs_to', 'curated_mapping', { origin: 'theme_literacy_ontology' });
            }
        }
    }
}
function seedSemanticEdges() {
    ...
    for (const subject of SUBJECTS) {
        ...
        for (const edge of mapping.prerequisites) {
            ...
            if (from && to)
                ensureEdge(from.id, to.id, 'prerequisite', 'curated_mapping', { origin: 'mapping_json' });
        }
    }
    ...
    for (const node of nodes) {
        ...
        if (!hasOutgoing(node.id, 'reinforces')) {
            for (const literacyCode of literacyCodes) {
                const literacy = getNodeByCode('literacy', literacyCode);
                if (literacy)
                    ensureEdge(node.id, literacy.id, 'reinforces', mapping.literacies.has(node.title) ? 'curated_mapping' : 'fallback_classification', { literacyCode });
            }
        }
...
function seedCurriculumSequenceEdges() {
    const scopes = db.prepare(`SELECT DISTINCT subject, grade FROM curriculum_textbooks ORDER BY subject, CAST(grade AS INTEGER)`).all();
    ...
    `).all(scope.subject, scope.grade);
      ...
      `).get(previous, current, current, previous);
            if (!connected)
                ensureEdge(previous, current, 'related_to', 'curriculum_sequence', { subject: scope.subject, grade: scope.grade, confidence: 0.35 });
        }
    }
}
function fillWeightGaps() {
    const scopes = db.prepare(`SELECT DISTINCT subject, grade FROM curriculum_textbooks ORDER BY subject, CAST(grade AS INTEGER)`).all();
    for (const scope of scopes)
        seedWeights(scope.subject, scope.grade);
...
/** 批量修复当前数据库；可安全重复执行。 */
export function repairProductionKnowledgeGraph() {
...
export function auditProductionKnowledgeGraph() {
  ...
  `).all();
    const scopeIssues = scopes.map((scope) => ({
  ...
  `).get().n;
    const selfEdges = db.prepare(`SELECT COUNT(*) AS n FROM kg_edges WHERE source_id=target_id`).get().n;
  ...
  `).get().n;
    const failures = [];
    if (integrity !== 'ok')
        failures.push(`SQLite integrity_check=${integrity}`);
    if (foreignKeyViolations)
        failures.push(`发现 ${foreignKeyViolations} 条外键违规`);
    if (legacyKnowledgePointsWithoutGraphNode)
        failures.push(`${legacyKnowledgePointsWithoutGraphNode} 个教材知识点未同步到图谱`);
    if (duplicateGraphKnowledgePoints)
        failures.push(`${duplicateGraphKnowledgePoints} 组图谱知识点标题重复`);
    if (selfEdges)
        failures.push(`${selfEdges} 条图谱自环`);
    if (crossSubjectKnowledgeEdges)
        failures.push(`${crossSubjectKnowledgeEdges} 条跨学科知识点边`);
    for (const scope of scopeIssues) {
        if (!scope.kps)
            failures.push(`${scope.subject} G${scope.grade} 没有可发布知识点`);
...
export function assertProductionKnowledgeGraph() {
    const audit = auditProductionKnowledgeGraph();
    if (audit.failures.length)
        throw new Error(`知识图谱未达到生产准入：${audit.failures.join('；')}`);
    return audit;
}
