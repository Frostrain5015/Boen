...
import { ingestTextbook, type TextbookSeed } from './curriculum.js';
...
const DATA_DIR = resolve(__dirname, '../curriculum');
...
async function main() {
  ...
  try {
    ...
    console.error(`找不到课程数据目录：${DATA_DIR}`);
    process.exit(1);
  }
  if (files.length === 0) {
    console.warn(`${DATA_DIR} 下没有 .json 课程数据`);
    return;
  }
  console.log(`课程数据目录：${DATA_DIR}，共 ${files.length} 个文件`);
  let totalUnits = 0;
  let totalKps = 0;
  for (const file of files) {
    const raw = JSON.parse(readFileSync(join(DATA_DIR, file), 'utf-8')) as TextbookSeed | TextbookSeed[];
