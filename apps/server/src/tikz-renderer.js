...
const renderRequestsByUser = new Map();
...
export function validateTikzCode(code) {
...
export function consumeTikzRateLimit(userId) {
    const now = Date.now();
    // 缓存上限保护：超出时清理不活跃用户
...
function documentFor(code) {
    ...
    const extraLibs = [];
    let body = code.replace(/\\usetikzlibrary\s*\{([^}]*)\}/g, (_, libs) => {
...
export async function renderTikzSvg(code, requestId) {
    ...
    try {
        ...
        if (error instanceof TikzRenderError)
            throw error;
