 ...
 */
// ── 通用辅助 ─────────────────────────────
const SUBJECT_LABELS = { chinese: '语文', math: '数学', english: '英语', science: '科学' };
...
/** 小学低年级竖式提示（仅二~四年级） */
export function xlopGuide(grade) {
    return (grade === '2' || grade === '3' || grade === '4')
        ? '\n列竖式计算用 \\opadd / \\opsub / \\opmul / \\opdiv 直接写在题干文本中（如 \\opadd{698}{213}），前端自动渲染为竖式。'
        : '';
}
// ── 阶段一：蓝图架构师 prompt ─────────────
export function blueprintArchitectPrompt(config, weightGuide, profileContext, constraintBoundary) {
...
export function questionWriterPrompt(ctx) {
...
export function formatQuestionSnapshot(questions) {
    return questions.map((q) => {
        ...
        if (q.explanation)
            lines.push(`  解析: ${q.explanation.slice(0, 150)}`);
        // 提取 TikZ 代码供审核
        const tikzBlocks = [
...
/** A. 正确性审核 */
export function reviewCorrectnessPrompt(questions, config) {
...
/** B. 相似性审核 */
export function reviewSimilarityPrompt(questions) {
    return [
...
/** C. 蓝图匹配审核 */
export function reviewBlueprintMatchPrompt(questions, blueprint) {
    const blueprintSummary = blueprint.sections.map(s => `  ${s.title}（难度${s.difficulty}）：${s.knowledgePoints.map(kp => kp.title).join('、')}`).join('\n');
...
/** D. 格式审核 */
export function reviewFormatPrompt(questions, config) {
    const isMathOrScience = config.subject === 'math' || config.subject === 'science';
...
/** E. 区分度审核 */
export function reviewDiscriminationPrompt(questions, config) {
...
// ── 阶段四：重出 prompt ───────────────────
export function regenerateQuestionPrompt(question, feedback, config, crossGroupContext) {
    return [
