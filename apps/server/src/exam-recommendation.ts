...
import type { ExamResults } from '@boen/shared';
...
export interface PostExamRecommendation {
  recommendedPractice: Array<{
    ...
    kpTitle: string;
    reason: string;
    suggestedMode: 'weakness' | 'review' | 'preview';
  }>;
  nextExamScope: { subject: string; grade: string; reason: string };
 ...
 */
export function postExamRecommendation(
  userId: string,
  subject: string,
  grade: string,
  results: ExamResults,
): PostExamRecommendation {
  const recommendedPractice: PostExamRecommendation['recommendedPractice'] = [];
  ...
  if (weakKps.length > 0) {
    ...
    for (const weakKp of weakKps) {
      ...
      `).all(node.id) as Array<{ source_id: number; title: string }>;
      ...
      if (prereqs.length > 0) {
        ...
        for (const prereq of prereqs) {
          const profRow = db.prepare(
            `SELECT weighted_score FROM user_kp_proficiency WHERE user_id = ? AND kg_node_id = ?`,
          ).get(userId, prereq.source_id) as { weighted_score: number } | undefined;
  ...
  const modePriority: Record<string, number> = { weakness: 0, review: 1, preview: 2 };
...
function computeDifficultyAdjustment(
  ...
  for (const row of recentExams.slice(0, 2)) {
    try {
      const parsed = JSON.parse(row.results) as { percentage?: number };
...
function computeNextExamScope(
  subject: string,
  grade: string,
  results: ExamResults,
  allHigh: boolean,
): { subject: string; grade: string; reason: string } {
  const subjectLabels: Record<string, string> = { chinese: '语文', math: '数学', english: '英语', science: '科学' };
  ...
  const weakest = results.kpBreakdown.reduce(
    (min, kp) => (kp.percentage < min.percentage ? kp : min),
    results.kpBreakdown[0],
  );
