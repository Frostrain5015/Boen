...
function loadKPNodes(subj: string, g: string): Array<{ id: number; title: string; description: string | null }> {
  ...
  const rows = db.prepare(`
    ...
    WHERE n.type = 'knowledge_point'
      ...
      AND t.grade = ?
    ORDER BY n.id
  `).all(subj, g) as Array<{ id: number; title: string; description: string | null }>;
  return rows;
}

/** 读取 curriculum JSON 获取教材结构上下文 */
function loadCurriculumContext(subj: string, g: string): string {
  ...
  for (const file of patterns) {
    ...
    try {
      ...
      const lines: string[] = [`## ${data.title || file}`];
...
/** 加载课标参考文本（如果存在） */
function loadStandardText(subj: string): string {
...
function loadLowerGradeKPs(subj: string, g: string): string[] {
...
const ThemeSchema = z.record(z.string(), z.enum(themeCodes as [string, ...string[]]));
const LiteracySchema = z.record(z.string(), z.array(z.enum(literacyCodes as [string, ...string[]])).min(1).max(3));
const BloomSchema = z.record(z.string(), z.enum(bloomCodes as [string, ...string[]]));
...
async function generateMappings(kpTitles: string[], context: string, standardText: string, lowerGradeKPs: string[]) {
...
6. 确保每个知识点都有 theme、literacy、bloom 映射。`;
  ...
  if (lowerGradeKPs.length > 0) {
    ...
    if (lowerGradeKPs.length > 50) userContent += `\n... 共${lowerGradeKPs.length}条`;
  }
...
function validateKPTitles(mappings: z.infer<typeof MappingsSchema>, validTitles: Set<string>, lowerGradeTitles: Set<string>) {
  ...
  const validPrereqs = mappings.prerequisites.filter(p => {
    ...
    if (!fromOk) warnings.push(`prerequisite.from 不存在: "${p.from}"`);
    if (!toOk) warnings.push(`prerequisite.to 不存在: "${p.to}"`);
    return fromOk && toOk;
  });

  for (const title of Object.keys(mappings.themes)) {
    if (!validTitles.has(title)) warnings.push(`theme 映射标题不存在: "${title}"`);
  }
  for (const title of Object.keys(mappings.literacies)) {
    if (!validTitles.has(title)) warnings.push(`literacy 映射标题不存在: "${title}"`);
  }
  for (const title of Object.keys(mappings.blooms)) {
    if (!validTitles.has(title)) warnings.push(`bloom 映射标题不存在: "${title}"`);
  }
