...
async function main() {
    ...
    console.log('现状: mistake_items=%d, style_skills=%d, style_features=%d', count('SELECT COUNT(*) c FROM mistake_items'), count('SELECT COUNT(*) c FROM style_skills'), count('SELECT COUNT(*) c FROM mistake_style_features'));
    // Step 2a：空壳题
    ...
    if (preview.sample.length)
        console.log('  样例:', preview.sample.map((s) => `「${s}」`).join('  '));
    if (dryRun) {
    ...
    if (!apiKey)
        throw new Error('缺少 BOEN_API_KEY / DEEPSEEK_API_KEY，无法调用 LLM 重建');
