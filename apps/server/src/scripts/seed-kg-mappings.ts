...
function getNodeByTitleSubject(title: string, subj: string): { id: number } | undefined {
  return db.prepare(
    `SELECT id FROM kg_nodes WHERE type='knowledge_point' AND subject=? AND title=?`
  ).get(subj, title) as { id: number } | undefined;
}

function getNodeByCode(type: string, code: string): { id: number } | undefined {
  return db.prepare(
    `SELECT id FROM kg_nodes WHERE type=? AND code=?`
  ).get(type, code) as { id: number } | undefined;
}
...
for (const edge of data.prerequisites) {
  ...
  if (!fromNode) { console.warn(`  ⚠ from not found: "${edge.from}"`); continue; }
  if (!toNode) { console.warn(`  ⚠ to not found: "${edge.to}"`); continue; }
  if (!dryRun) insEdge.run(fromNode.id, toNode.id, 'prerequisite', 1.0);
  counts.prerequisite++;
}
...
for (const [title, themeCode] of Object.entries(data.themes)) {
  ...
  const themeNode = getNodeByCode('theme', themeCode as string);
  if (!kpNode) { console.warn(`  ⚠ KP not found: "${title}"`); continue; }
  if (!themeNode) { console.warn(`  ⚠ theme not found: "${themeCode}"`); continue; }
  if (!dryRun) insEdge.run(kpNode.id, themeNode.id, 'belongs_to', 1.0);
  counts.theme++;
}
...
for (const [title, litCodes] of Object.entries(data.literacies)) {
  const kpNode = getNodeByTitleSubject(title, subject);
  if (!kpNode) { console.warn(`  ⚠ KP not found: "${title}"`); continue; }
  for (const litCode of litCodes as string[]) {
    const litNode = getNodeByCode('literacy', litCode);
    if (!litNode) { console.warn(`  ⚠ literacy not found: "${litCode}"`); continue; }
    if (!dryRun) insEdge.run(kpNode.id, litNode.id, 'reinforces', 1.0);
    counts.literacy++;
  }
}
...
for (const [title, bloomCode] of Object.entries(data.blooms)) {
  const kpNode = getNodeByTitleSubject(title, subject);
  const bloomNode = getNodeByCode('bloom_level', bloomCode as string);
  if (!kpNode) { console.warn(`  ⚠ KP not found: "${title}"`); continue; }
  if (!bloomNode) { console.warn(`  ⚠ bloom not found: "${bloomCode}"`); continue; }
  if (!dryRun) insEdge.run(kpNode.id, bloomNode.id, 'bloom_at', 1.0);
  counts.bloom++;
}
