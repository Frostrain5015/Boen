...
import { stepBlueprintArchitect, flattenBlueprint } from './exam-blueprint.js';
import { reviewBoard, regenerateQuestions } from './exam-reviewers.js';
import { questionWriterPrompt } from './exam-prompts.js';
import { canonicalizeStoredQuestionTaxonomy, getPublishedKnowledgePointIds, resolveQuestionTaxonomy } from './question-taxonomy.js';
// ── 通用加固工具 ──────────────────────────
/** 把 AI 可能返回的字符串格式 literacies 归一化为数组 */
function normalizeLiteracies(raw) {
    if (Array.isArray(raw))
        return raw.filter((l) => typeof l === 'string');
 ...
 */
function canonicalizeExamBlueprint(blueprint, config) {
    ...
    const resolve = (candidate) => resolveQuestionTaxonomy({
    ...
    const resolveName = (value) => resolve({ title: value })?.knowledgePoint ?? null;
    const sections = blueprint.sections.map((section, sectionIndex) => {
        ...
        if (!knowledgePoints.length)
            throw new Error(`蓝图第 ${sectionIndex + 1} 个板块没有有效知识点`);
        return {
            ...section,
            knowledgePoints,
            questionTypes: section.questionTypes.map((plan) => {
                const focusKps = [...new Set(plan.focusKps.map(resolveName).filter((name) => Boolean(name)))];
                return { ...plan, focusKps: focusKps.length ? focusKps : [knowledgePoints[0].title] };
            }),
        };
    });
    const canonicalNames = (values) => [...new Set((values ?? []).map(resolveName).filter((name) => Boolean(name)))];
    const coveragePlan = {
...
const TYPE_LABELS = { multiple_choice: '选择题', fill_blank: '填空题', true_false: '判断题', short_answer: '简答题' };
 ...
 */
function calibratePointsToBlueprint(questions, blueprint) {
...
function orderExamQuestionsForDelivery(questions) {
...
function isPlaceholderOptionText(text, key) {
...
function normalizeOptions(stem, rawOptions) {
    ...
    // 超过 2 个有效选项 → 直接用
    if (valid.length >= 2)
        return { stem: cleanedStem, options: valid };
...
function normalizeBlanks(raw, stem) {
    ...
    while (blanks.length < blankCount)
        blanks.push({ acceptedAnswers: [''] });
    return { blanks, blankCount };
}
function choiceWithoutOptionsToShortAnswer(q) {
...
function localFormatFix(q, i) {
    ...
    if (q2.type === 'multiple_choice') {
        ...
        if (!q2.correctKeys?.length)
            q2.correctKeys = [q2.options[0].key];
...
function parseGeneratedQuestionList(raw, questionType, count) {
    const rawQuestions = raw?.questions ?? (Array.isArray(raw) ? raw : []);
...
function validateExamQuestionForDelivery(q) {
    if (!q.stem || q.stem.trim().length < 5)
        return '题干过短或缺失';
    if (!q.explanation || q.explanation.trim().length < 5)
        return '解析过短或缺失';
    if (!Number.isInteger(q.knowledgePointId) || (q.knowledgePointId ?? 0) <= 0)
        return 'knowledgePointId 缺失';
    if (!q.knowledgePoint || q.knowledgePoint.trim().length === 0)
        return 'knowledgePoint 缺失';
    if (!normalizeLiteracies(q.literacies).length)
        return 'literacies 缺失';
    if (q.type === 'multiple_choice') {
        if (!q.options || q.options.length < 3)
            return `选择题选项不足（${q.options?.length ?? 0} 个）`;
        if (!q.correctKeys?.length)
            return '选择题正确答案缺失';
        if (q.options.some(o => isPlaceholderOptionText(o.text, o.key)))
            return '选择题选项存在占位符';
    }
    if (q.type === 'fill_blank') {
        ...
        if (!q.blanks?.length)
            return '填空题答案结构缺失';
        if (q.blanks.length !== markerCount)
            return `填空题空位数与答案数不一致（空位 ${markerCount}，答案 ${q.blanks.length}）`;
    }
    if (q.type === 'true_false' && typeof q.answer !== 'boolean')
        return '判断题 answer 缺失';
    if (q.type === 'short_answer') {
        if (!q.referenceAnswer?.trim() && !(q.keyPoints?.length))
            return '简答题缺少 referenceAnswer/keyPoints';
    }
    return null;
}
function normalizeOptionalPassage(raw) {
    if (typeof raw !== 'string')
        return undefined;
    const text = cleanGeneratedText(raw);
    if (!text)
        return undefined;
    if (/有阅读材料时填写|无则省略|阅读材料时填写|无则省略此字段/.test(text))
        return undefined;
    return text;
}
// ── 考试生成（四阶段并发流水线） ──────────────
...
async function renderTikzBlockAsync(texCode) {
 ...
 */
export async function generateExam(model, config, onProgress, userId) {
    try {
        ...
        const enrichedConfig = { ...config, styleContext };
        await onProgress?.({ step: 'blueprint', message: 'blueprint', progress: 10 });
        const rawBlueprint = await stepBlueprintArchitect(model, { subject: enrichedConfig.subject, grade: enrichedConfig.grade, totalScore: enrichedConfig.totalScore, notes: enrichedConfig.notes }, weightGuide, [curriculumContext, profileContext, styleContext].filter(Boolean).join('\n\n'), mode);
        ...
        const writeResults = await withConcurrencyLimit(writeTasks.map((task) => async () => {
        ...
        for (const r of writeResults) {
            if (r.status === 'fulfilled')
                ...
                console.error(`出题组失败:`, r.reason?.message?.slice(0, 100));
        }
        // 编号 + 本地格式修复 + 卷面题型顺序硬约束：
        ...
        const regeneratedIndices = new Set();
        ...
        const regenHistory = [regenCount]; // 追踪每轮未通过数，用于趋势检测
        for (let round = 1; regenCount > 0 && round <= maxRegenerationRounds; round++) {
            ...
            const regenResult = await regenerateQuestions(model, questions, scores, { subject: enrichedConfig.subject, grade: enrichedConfig.grade }, crossGroupContext, (writerModel, prompt, questionType) => writeSingleQuestion(writerModel, prompt, questionType, enrichedConfig));
            questions = orderExamQuestionsForDelivery(regenResult.questions);
            for (const index of regenResult.report.regeneratedIndices)
                regeneratedIndices.add(index);
            for (const index of regenResult.report.qualityWarnings)
                qualityWarnings.add(index);
...
function buildCrossGroupContext(allQuestions) {
 ...
 */
async function stepWriteQuestionsV2(model, config, task, blueprint, allTasks, existingQuestions) {
    // 跨组差异化上下文：其他组的知识点 + 题型
    ...
    if (!fallbackTaxonomy)
        throw new Error('出题板块没有有效的数据库知识点，不能生成占位题');
...
async function invokeGenerateQuestions(model, prompt, questionType, count) {
    const examples = {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            const response = await model.invoke([new SystemMessage(textPrompt)], { response_format: { type: 'json_object' } });
...
function toExamQuestion(raw, task, index, config) {
    ...
    if (base.type === 'multiple_choice') {
        ...
        base.correctKeys = (raw.correctKeys ?? []).filter((k) => base.options.some(o => o.key === k));
        if (!base.correctKeys.length)
            base.correctKeys = [base.options[0].key];
    ...
    if (base.type === 'true_false')
        base.answer = raw.answer ?? true;
    if (base.type === 'short_answer') {
        base.referenceAnswer = raw.referenceAnswer ?? '';
        base.keyPoints = raw.keyPoints ?? [];
    }
    return base;
}
// ── 重出阶段：单题出题（供 regenerateQuestions 回调） ─────────
async function writeSingleQuestion(model, prompt, questionType, config) {
    try {
...
// ── 学情画像上下文（保留原有逻辑） ──────────────────────────
async function buildProfileContext(userId, config) {
    ...
    const parts = [];
    if (weak.length)
        parts.push('薄弱知识点（应优先考查）：' + weak.map(w => w.title).join('、'));
    if (recs.length)
        parts.push('推荐强化知识点：' + recs.map(r => r.title).join('、'));
    return parts.join('\n');
}
function buildWeightGuideForPrompt(dist) {
    const byTier = {};
    for (const d of dist) {
        if (!byTier[d.tier])
            byTier[d.tier] = [];
...
function sanitizeUserAnswer(raw) {
...
function buildGradingHumanMessage(params) {
    return [
        `【题目】${params.stem}`,
...
export function createShortAnswerGrader(model) {
    return async (params) => {
        try {
 ...
 * 如果批量失败，回退到逐题评分。
 */
export async function batchGradeShortAnswers(model, items) {
    if (items.length === 0)
        return [];
    if (items.length === 1) {
        // 单题直接走单题模式
...
/** 批量评分回退：逐题调用 */
async function fallbackBatchGrade(model, items) {
    const grader = createShortAnswerGrader(model);
    const results = [];
    for (const item of items) {
 ...
 * 仅对未通过的空白触发，不会成为常态开销。
 */
async function batchCheckBlanksLevel3(model, items) {
...
请输出 JSON：{"results": [{"questionIndex": 0, "blankIndex": 0, "match": true, "reason": "简短理由"}]}`;
    ...
    try {
        const response = await model.invoke([new SystemMessage(systemPrompt), new HumanMessage(humanContent)], { response_format: { type: 'json_object' } });
        const content = typeof response.content === 'string' ? response.content : '';
        const parsed = JSON.parse(content);
        const results = Array.isArray(parsed?.results) ? parsed.results : [];
...
async function generateExamAnalysis(model, questions, results, config) {
    const subjectLabel = { chinese: '语文', math: '数学', english: '英语', science: '科学' };
    ...
    const questionDetails = results.questionResults.map((qr) => {
        ...
        const typeLabel = { multiple_choice: '选择', fill_blank: '填空', true_false: '判断', short_answer: '简答' };
...
// ── 考试评分 ─────────────────────────────────
export async function gradeExam(questions, answers, model, examInfo, onProgress, options) {
    // ── 断点续判：构建已判题目集合，计算进度偏移 ──
    const existingResultsMap = new Map();
    if (options?.existingResults) {
        for (const r of options.existingResults)
            existingResultsMap.set(r.index, r);
    }
    ...
    const objectiveResults = await Promise.all(objectiveQuestions.map(async (q) => {
        ...
        if (q.type === 'fill_blank' && answer.type === 'fill_blank') {
            ...
            result.__blankLevel2Misses = blankLevel2Misses;
        }
    ...
    if (model) {
        ...
        for (const r of objectiveResults) {
            const misses = r.__blankLevel2Misses;
            if (misses?.length) {
    ...
    if (shortAnswerQuestions.length > 0 && model) {
        const batchItems = shortAnswerQuestions.map(q => ({
        ...
        // 无模型时简答题不给分
        for (const q of shortAnswerQuestions) {
            shortAnswerResults.push({ index: q.index, correct: null, score: 0, maxScore: q.points, reference: q.referenceAnswer ?? '', explanation: q.explanation || '评分服务暂时不可用，请老师人工批阅。', knowledgePoint: q.knowledgePoint, knowledgePointId: q.knowledgePointId, literacy: normalizeLiteracies(q.literacies) });
        }
    }
    // ── 简答题批改完成，保存断点 ──
    ...
    for (const r of existingResultsMap.values())
        allResultsMap.set(r.index, r);
    for (const r of objectiveResults)
        allResultsMap.set(r.index, r);
...
function auditGradingResults(questions, results) {
    const issues = [];
    // 1. 总分范围校验
    const totalScore = results.reduce((s, r) => s + r.score, 0);
    const maxScore = results.reduce((s, r) => s + r.maxScore, 0);
    if (totalScore > maxScore)
        issues.push(`总分 ${totalScore} 超过满分 ${maxScore}`);
    if (totalScore < 0)
        issues.push(`总分 ${totalScore} 为负`);
    // 2. 逐题分数范围校验
    for (const r of results) {
...
function computeTierBreakdown(questions, results) {
    const tiers = { Core: { correct: 0, total: 0 }, Important: { correct: 0, total: 0 }, Standard: { correct: 0, total: 0 }, Other: { correct: 0, total: 0 } };
    for (const q of questions) {
        const r = results.find(x => x.index === q.index);
        const tier = getKpTier(q.knowledgePoint ?? '');
        if (!tiers[tier])
            tiers[tier] = { correct: 0, total: 0 };
...
function computeKpBreakdown(results) {
    const map = new Map();
    for (const r of results) {
        if (!r.knowledgePoint)
            continue;
...
function computeLiteracyBreakdown(results) {
    const map = new Map();
    for (const r of results) {
 ...
 */
export function findKnowledgePointNode(title, subject) {
    const subjectSql = subject ? ` AND subject=?` : '';
    // Level 1: 精确匹配
    let node = db.prepare(`SELECT id, weight FROM kg_nodes WHERE type='knowledge_point' AND title=?${subjectSql}`).get(...(subject ? [title, subject] : [title]));
    if (node)
        return node;
    // Level 2: 查询词被包含在库标题中（"一般现在时" → "一般现在时的用法"）
    node = db.prepare(`SELECT id, weight FROM kg_nodes WHERE type='knowledge_point' AND title LIKE ?${subjectSql}`).get(...(subject ? [`%${title}%`, subject] : [`%${title}%`]));
    if (node)
        return node;
    // Level 3: 库标题被包含在查询词中（反向）
    node = db.prepare(`SELECT id, weight FROM kg_nodes WHERE type='knowledge_point' AND ? LIKE '%' || title${subjectSql}`).get(...(subject ? [title, subject] : [title]));
    if (node)
        return node;
    return undefined;
}
function getKpTier(kp, subject) {
    const node = findKnowledgePointNode(kp, subject);
    if (!node)
        return 'Standard';
    if (node.weight && node.weight >= 0.75)
        return 'Core';
    if (node.weight && node.weight >= 0.5)
        return 'Important';
    return 'Standard';
}
function questionTypeToToolName(type) {
    const map = { multiple_choice: 'ask_multiple_choice', fill_blank: 'ask_fill_blank', true_false: 'ask_true_false', short_answer: 'ask_short_answer' };
    return map[type] || 'ask_multiple_choice';
}
function buildRawArgs(q) {
    const base = { stem: q.stem, explanation: q.explanation ?? '', knowledgePoint: q.knowledgePoint, knowledgePointId: q.knowledgePointId, literacies: normalizeLiteracies(q.literacies), difficulty: q.difficulty };
    if (q.type === 'multiple_choice')
        return { ...base, options: q.options, correctKeys: q.correctKeys, multiSelect: q.multiSelect };
    if (q.type === 'fill_blank')
        return { ...base, blanks: q.blanks };
    if (q.type === 'true_false')
        return { ...base, answer: q.answer };
    if (q.type === 'short_answer')
        return { ...base, referenceAnswer: q.referenceAnswer, keyPoints: q.keyPoints };
    return base;
}
// ── 针对性错误详解（按需触发） ────────────────────────────
/**
 ...
 * 不增加默认判分流程的延迟。
 */
export async function generateDetailedReview(model, questions, answers, results) {
    // Collect wrong objective questions (multiple_choice, fill_blank, true_false)
    const wrongObjectives = results.questionResults.filter((qr) => qr.correct === false && qr.maxScore > 0);
    if (wrongObjectives.length === 0)
    ...
    const questionContexts = wrongObjectives.map((qr) => {
        ...
        if (answer) {
            if (answer.type === 'multiple_choice')
                studentAnswerText = answer.selectedKeys?.join(', ') ?? '（未作答）';
            else if (answer.type === 'fill_blank')
                studentAnswerText = answer.answers?.join(' / ') ?? '（未作答）';
            else if (answer.type === 'true_false')
                studentAnswerText = answer.value ? '正确' : '错误';
        }
        const typeLabel = { multiple_choice: '选择题', fill_blank: '填空题', true_false: '判断题' };
        return {
            index: qr.index,
    ...
    const objectiveContexts = questionContexts.filter((ctx) => ctx.type === '选择题' || ctx.type === '填空题' || ctx.type === '判断题');
    if (objectiveContexts.length === 0)
        return results.questionResults;
...
输出 JSON：{"results": [{"index": 0, "explanation": "针对性解析"}]}`;
    ...
    try {
        ...
        const content = typeof response.content === 'string' ? response.content : '';
        const parsed = JSON.parse(content);
        const detailedResults = Array.isArray(parsed?.results) ? parsed.results : [];
 ...
 */
async function autoCollectMistakes(userId, examId, questions, answers, results, subject, grade, model) {
    const mistakeIds = [];
    const answerMap = new Map(answers.map(a => [a.questionIndex, a.answer]));
    // 筛选得分率 < 60% 的题目
    const penalized = results.questionResults.filter((qr) => qr.maxScore > 0 && qr.score / qr.maxScore < 0.6);
    if (penalized.length === 0)
        return { count: 0, mistakeIds };
    ...
    if (model && penalized.length > 0) {
        try {
            ...
            const response = await model.invoke([new SystemMessage('你是错题分析助手，只输出可解析 JSON。'), new HumanMessage(prompt)], { response_format: { type: 'json_object' } });
            const content = typeof response.content === 'string' ? response.content : '';
            const parsed = JSON.parse(content);
            analyses = (Array.isArray(parsed) ? parsed : parsed.results ?? parsed.analyses ?? []).map((a) => ({
    ...
    for (let i = 0; i < penalized.length; i++) {
        ...
        if (userAnswer) {
            if (userAnswer.type === 'multiple_choice')
                studentAnswerText = userAnswer.selectedKeys?.join(', ') ?? null;
            else if (userAnswer.type === 'fill_blank')
                studentAnswerText = userAnswer.answers?.join(' | ') ?? null;
            else if (userAnswer.type === 'true_false')
                studentAnswerText = userAnswer.value ? '正确' : '错误';
            else if (userAnswer.type === 'short_answer')
                studentAnswerText = userAnswer.text ?? null;
        }
        // 答案匹配度：基于实际得分率
    ...
    `).run(id, userId, subject, grade, title, q.stem?.slice(0, 8000) ?? '', studentAnswerText, correctAnswer ?? null, q.explanation?.slice(0, 2000) ?? null, errorType ?? null, errorReason ?? null, analysis ? 0.7 : 0.3, matchScore, 0, // is_correct = false (heavily penalized)
        now, now);
        ...
        if (kpTitle && results.proficiencyChanges) {
            ...
            if (qBefore) {
                ...
                if (node) {
                    const profRow = db.prepare('SELECT weighted_score FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, node.id);
...
export function updateExamSession(examId, userId, config, data) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare(`UPDATE exam_sessions SET subject=?, grade=?, title=?, questions=?, total_score=?, duration_minutes=?, status='pending', updated_at=?, blueprint=?, quality_report=? WHERE id=? AND user_id=?`).run(config.subject, config.grade, data.title, JSON.stringify(data.questions), data.totalScore, data.durationMinutes, now, JSON.stringify(data.blueprint), JSON.stringify(data.qualityReport), examId, userId);
    return {
        id: examId,
        userId,
        subject: config.subject,
...
export function getExamSession(examId, userId) {
    const row = db.prepare(`SELECT * FROM exam_sessions WHERE id=? AND user_id=?`).get(examId, userId);
    if (!row)
        return null;
    const questions = JSON.parse(row.questions).map((q, i) => {
...
export function deleteExamSession(examId, userId) {
...
export function listExamSessions(userId) {
    const rows = db.prepare(`SELECT id, subject, grade, title, total_score, status, created_at, submitted_at, results
     FROM exam_sessions
     WHERE user_id=? AND status='completed'
     ORDER BY COALESCE(submitted_at, created_at) DESC`).all(userId);
    return rows.map((r) => {
        const results = r.results ? JSON.parse(r.results) : undefined;
        return {
            examId: r.id,
            title: r.title,
            subject: r.subject,
...
function readGradingCheckpoint(examId) {
    const row = db.prepare(`SELECT grading_checkpoint FROM exam_sessions WHERE id=?`).get(examId);
    if (!row?.grading_checkpoint)
        return null;
    try {
...
export async function submitExamSession(examId, userId, answers, model, onProgress) {
    ...
    const results = await gradeExam(session.questions, answers, model, { subject: session.subject, grade: session.grade }, onProgress, { examId, existingResults: checkpoint ?? undefined });
    await onProgress?.({ step: 'profile', message: 'profile', progress: 84 });
    // 更新知识画像并记录变化（优先用 knowledgePointId 直连节点）
    const proficiencyChanges = [];
    for (const qr of results.questionResults) {
        // 收集要更新的节点
        const nodesToUpdate = [];
        if (qr.knowledgePointId) {
            const node = db.prepare('SELECT id, title FROM kg_nodes WHERE id=?').get(qr.knowledgePointId);
            if (node)
        ...
        if (nodesToUpdate.length === 0 && qr.knowledgePoint) {
            for (const kp of qr.knowledgePoint.split(/[；;]/).map(s => s.trim()).filter(Boolean)) {
                ...
                if (node)
                    nodesToUpdate.push({ id: node.id, title: kp });
            }
        }
        for (const node of nodesToUpdate) {
            const oldRow = db.prepare('SELECT weighted_score FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, node.id);
            const before = oldRow?.weighted_score ?? -1;
            updateProficiency(userId, node.id, qr.score, qr.maxScore, 'exam');
            const newRow = db.prepare('SELECT weighted_score FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, node.id);
    ...
    if (proficiencyChanges.length)
        results.proficiencyChanges = proficiencyChanges;
    // ── 星月积分结算：评分(主) + 星级跨越奖(额外) ──
    try {
        const outline = getProfileOutline(session.subject, String(session.grade ?? ''), userId);
    ...
    try {
        ...
        const collected = await autoCollectMistakes(userId, examId, session.questions, answers, results, session.subject, session.grade, model);
