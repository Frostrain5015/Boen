...
import { getChatModel, buildBoenGraph, QUIZ_TOOL_NAMES, COMPLETE_REVIEW_TOOL, toQuestionPayload, gradeAnswer, EXIT_SESSION_TOOL, ADVANCE_STEP_TOOL, PLAN_STEPS_TOOL, SWITCH_SUBJECT_TOOL, LOOKUP_KNOWLEDGE_POINT_TOOL, } from '@boen/agent-core';
...
import { updateProficiency, cacheProficiencyUpdate, flushProficiencyCache, getCachedProficiencySum, getCachedProficiencyExpected, setCachedProficiencyExpected, computeProficiencyDelta, difficultyLevelToValue, expectedCorrectness, ELO_RATING_INIT, ELO_SIGMA_INIT, getAllProficiencies, getWeakPoints, getStrongPoints, getLiteracyProficiency, getRecommendedKPs, getPrerequisiteWeaknessChain, getProfileOutline, seedProficiencyFromHistory } from './knowledge-profile.js';
import { createConversation, getConversations, getConversation, updateConversationTitle, deleteConversation, addMessage, getMessages, updateQuestionMessage, } from './conversation.js';
import { analyzeMistake, archiveMistake, createMistake, formatMistakePracticePrompt, getMistakeAssetFile, getMistakeDetail, listMistakes, readMistakeAsset, retrieveGlobalStyleSkills, updateMistake, } from './mistakes.js';
...
const EXAM_MODEL_OPTS = { thinking: true, maxTokens: 8192, timeout: 300000 };
function createModel(provider, opts) {
...
if (!REDEEM_CODE_SECRET)
    console.warn('[redeem] REDEEM_CODE_SECRET 未配置，兑换码功能将拒绝服务');
    ...
    try {
        ...
        if (userIdCache.size >= MAX_USER_CACHE) {
            ...
            for (let i = 0; i < Math.max(1, Math.floor(MAX_USER_CACHE * 0.1)); i++) {
                ...
                if (tokens) {
                    tokens.delete(entries[i][0]);
                    if (!tokens.size)
                        userIdTokens.delete(oldSub);
                }
            }
        }
        ...
        if (!tokens) {
            tokens = new Set();
            userIdTokens.set(data.sub, tokens);
        }
        tokens.add(token);
        return data.sub;
    }
...
function checkRedeemRate(userId) {
    const now = Date.now();
...
async function resolveSubscription(c) {
    ...
    if (cached && cached.exp > Date.now() && cached.subscription) {
        return { userId: cached.sub, sub: cached.subscription };
    }
    ...
    if (!userId)
        return null;
    // 查订阅表
    const row = db.prepare('SELECT tier, expires_at, activated_at FROM subscriptions WHERE user_id = ?').get(userId);
    const now = Math.floor(Date.now() / 1000);
    const sub = {
        tier: (row?.tier === 'monthly' ? 'monthly' : row?.tier === 'yearly' ? 'yearly' : 'free'),
...
function requirePremium(c, result) {
    if (!result)
...
function getDailyUsage(userId) {
    ...
    const row = db.prepare('SELECT message_count FROM daily_chat_usage WHERE user_id = ? AND date = ?').get(userId, today);
...
const runConfig = (threadId) => ({ version: 'v2', configurable: { thread_id: threadId }, recursionLimit: 50 });
    ...
    for (const task of state.tasks ?? []) {
        for (const interrupt of task.interrupts ?? []) {
            ...
            if (value?.type === 'question'
                ...
                return { id: value.toolCallId, name: value.toolName, args: value.args };
            }
        }
    }
    return undefined;
 ...
 */
async function sendTodoPlanFromCommand(command, send) {
...
async function runGraph(input, threadId, send) {
    ...
    const looksLikeQuiz = (n) => !!n && (QUIZ_TOOL_NAMES.has(n) || n.startsWith('ask_'));
    ...
    try {
        for await (const ev of events) {
            if (ev.event === 'on_chat_model_stream') {
                ...
                if (text)
                    await send({ type: 'token', value: text });
                ...
                for (const name of toolNames) {
                    ...
                    if (name === EXIT_SESSION_TOOL && !todoStepSent.has(name)) {
                        ...
                        const args = chunk?.tool_calls?.[0]?.args ?? chunk?.tool_call_chunks?.[0] ?? {};
                ...
                if (!quizSignaled) {
                    ...
                    if ((out?.tool_calls ?? []).some((t) => looksLikeQuiz(t.name)))
                        await signalQuiz();
                }
                ...
                if (!todoStepSent.has(EXIT_SESSION_TOOL)) {
                    ...
                    if (exitCall) {
                        const total = ((Date.now() - stepTimestamps[0]) / 1000).toFixed(1);
                        console.log(`[Boen 类课堂] ✅ exit_session — ${stepCount}/${exitCall.args?.totalSteps ?? '?'}步 | ${exitCall.args?.score ?? '?'}分 | 总耗时 ${total}s | ${new Date().toLocaleTimeString()}`);
                    }
                }
                ...
                if (switchCall?.args) {
                    const subject = String(switchCall.args?.subject ?? 'math');
...
function awardSessionPoints(userId, changes, subject, grade, reason, refId, sessionScore) {
    try {
        let S = 0;
        if (subject && grade) {
...
async function handleSessionExit(last, send, userId, threadId, subject, grade) {
    let exitCall;
    // Case 1: last 本身是带 exit_session 的 AI 消息
    const lastCalls = (last?.tool_calls ?? []);
        ...
        try {
            ...
            for (let i = msgs.length - 1; i >= 0; i--) {
                ...
                if (aiMsg?.tool_calls?.length) {
                    const found = aiMsg.tool_calls.find((c) => c.name === EXIT_SESSION_TOOL);
                    if (found) {
                        exitCall = found;
                        break;
                    }
...
async function emitReviewCompleteIfAny(last, send) {
    const calls = (last?.tool_calls ?? []);
    const reviewCall = calls.find((c) => c.name === COMPLETE_REVIEW_TOOL);
    if (reviewCall?.args) {
        const args = reviewCall.args;
        await send({
...
/** 为新对话自动生成标题（基于首轮用户消息） */
async function autoGenerateTitle(conversationId, userMessage, onTitle) {
    try {
 ...
 */
function databaseQuestionPayload(quiz, scope) {
 ...
 */
async function resumeQuestionWithError(threadId, toolCallId, reason, send) {
    const { question } = await runGraph(new Command({
        resume: {
 ...
 * @returns 是否最终成功下发了题卡
 */
async function safeDeliverQuestion(quiz, send, scope, threadId, persistConversationId, maxRetry = 2) {
    let current = quiz;
    for (let attempt = 0; attempt <= maxRetry; attempt++) {
        if (!current?.id)
            return false; // 模型未再出题（如改为文字回复）→ 不再处理
        let payload = null;
        let reason = '';
        try {
 ...
 */
async function resumePendingQuestionAsSkipped(threadId, send) {
    const state = await graph.getState({ configurable: { thread_id: threadId } });
    const question = getPendingQuestion(state);
    if (!question?.id)
        return false;
    await runGraph(new Command({
        resume: {
            type: 'skip',
 ...
 */
async function drainPendingQuestions(threadId, send, max = 3) {
    for (let i = 0; i < max; i++) {
        let cleared = false;
        try {
...
function sanitizeConversationQuestionMessage(message, subject) {
...
const app = new Hono();
app.use('/api/*', cors());
app.get('/api/health', (c) => c.json({ ok: true, provider: 'deepseek', model: model?.modelName ?? 'deepseek-v4-flash' }));
// ── 模型切换 API ────────────────────────────
...
app.post('/api/model/switch', async (c) => {
    ...
    if (!p || !DEEPSEEK_MODELS[p])
        return c.json({ error: '不支持的 provider' }, 400);
    // V4 Pro 仅星耀卡可用
    if (p === 'deepseek-pro') {
        const row = db.prepare('SELECT tier FROM subscriptions WHERE user_id=?').get(userId);
        if (row?.tier !== 'yearly')
            return c.json({ error: 'premium_required', message: 'DeepSeek V4 Pro 仅限星耀卡用户使用' }, 403);
    }
...
app.post('/api/render-tikz', async (c) => {
    ...
    try {
        ...
        if (err instanceof TikzRenderError)
...
app.get('/api/kg/nodes', (c) => {
    ...
    if (!type)
        return c.json({ error: 'type 参数必填' }, 400);
...
app.get('/api/kg/neighbors/:nodeId', (c) => {
    ...
    if (isNaN(nodeId))
        return c.json({ error: 'nodeId 无效' }, 400);
...
app.get('/api/kg/unit/:unitId', (c) => {
    ...
    if (isNaN(unitId))
        return c.json({ error: 'unitId 无效' }, 400);
    ...
    try {
        ...
        if (contentType.includes('multipart/form-data')) {
            ...
            const mistake = createMistake(userId, {
                sourceType: formString(form, 'sourceType') || (asset ? 'image' : 'text'),
...
app.post('/api/mistakes/:id/analyze', async (c) => {
    ...
    return streamSSE(c, async (stream) => {
        const send = (e) => stream.writeSSE({ data: JSON.stringify(e) });
        try {
            await analyzeMistake(mistakeId, userId, model, (p) => send({ type: 'mistake_progress', step: p.step, message: p.message, progress: p.progress }), (mistake) => send({ type: 'mistake_ready', mistake }));
...
app.post('/api/exam/generate', async (c) => {
    ...
    if (!body.subject || !body.grade)
        return c.json({ error: '缺少必填字段：subject, grade' }, 400);
    ...
    return streamSSE(c, async (stream) => {
        ...
        try {
            ...
            const exam = await generateExam(examModel, { subject: body.subject, grade: body.grade, durationMinutes: body.durationMinutes, notes: body.notes, totalScore: body.totalScore }, (p) => send({ type: 'exam_progress', step: p.step, message: p.message, progress: p.progress ?? 0 }), userId);
...
app.post('/api/exam/submit', async (c) => {
    ...
    const body = await c.req.json();
    if (!body.examId)
        return c.json({ error: '缺少必填字段：examId' }, 400);
    // 允许 answers 为空数组（用户可能留空全部题目），未作答的题由评分逻辑按 0 分处理
    const answers = Array.isArray(body.answers) ? body.answers : [];
    try {
        const results = await submitExamSession(body.examId, userId, answers, model);
...
app.post('/api/exam/submit/stream', async (c) => {
    ...
    return streamSSE(c, async (stream) => {
        const send = (e) => stream.writeSSE({ data: JSON.stringify(e) });
        try {
            const results = await submitExamSession(body.examId, userId, answers, model, (p) => send({ type: 'exam_grading_progress', step: p.step, message: p.message, progress: p.progress }));
            // Attach post-exam adaptive recommendations
            try {
                const session = getExamSession(body.examId, userId);
                if (session) {
...
app.post('/api/exam', async (c) => {
    ...
    const now = Math.floor(Date.now() / 1000);
    db.prepare(`INSERT INTO exam_sessions (id, user_id, subject, grade, title, questions, total_score, duration_minutes, status, created_at, updated_at) VALUES (?, ?, ?, ?, '', '[]', 0, ?, 'generating', ?, ?)`).run(id, userId, body.subject, body.grade, body.durationMinutes ?? 45, now, now);
...
app.post('/api/exam/:examId/detailed-review', async (c) => {
    ...
    if (session.status !== 'completed')
        return c.json({ error: '考试尚未完成' }, 400);
    if (!session.results || !session.answers)
        return c.json({ error: '暂无判分结果' }, 400);
    try {
        const enhancedResults = await generateDetailedReview(model, session.questions, session.answers, session.results);
...
app.post('/api/subscription/redeem', async (c) => {
    if (!REDEEM_CODE_SECRET)
        return c.json({ error: 'unavailable', message: '兑换功能未启用' }, 503);
    const userId = await resolveUserId(c);
    if (!userId)
        return c.json({ error: 'unauthorized' }, 401);
    if (!checkRedeemRate(userId))
        return c.json({ error: 'rate_limited', message: '尝试过于频繁，请稍后再试' }, 429);
    let code = '';
    try {
        code = String((await c.req.json()).code ?? '').trim();
    }
    catch { /* 忽略 body 解析错误，下方按空码处理 */ }
    if (!code)
        return c.json({ error: 'invalid_code', message: '请输入兑换码' }, 400);
...
app.post('/api/currency/redeem-membership', async (c) => {
    ...
    let productKey = '';
    try {
        productKey = String((await c.req.json()).productKey ?? '').trim();
    }
    catch { /* 忽略 body 解析错误 */ }
...
app.post('/api/currency/claim-free-card', async (c) => {
    ...
    if (!checkRedeemRate(userId))
        return c.json({ error: 'rate_limited', message: '尝试过于频繁，请稍后再试' }, 429);
    try {
        const row = db.prepare('SELECT tier, activated_at FROM subscriptions WHERE user_id=?').get(userId);
        // 已激活过会员的不可再领
...
app.post('/api/chat', async (c) => {
    ...
    const owned = !!body.conversationId && getConversation(body.conversationId)?.userId === userId;
    return streamSSE(c, async (stream) => {
        const send = (e) => stream.writeSSE({ data: JSON.stringify(e) });
        try {
            ...
            const titlePromise = owned && getConversation(body.conversationId)?.title === '新对话'
            ...
            if (body.mode === 'weakness' && userId) {
                try {
                    ...
                    if (weakPoints.length > 0) {
                        weaknessData = '【你的薄弱知识点（来自知识画像）】\n' + weakPoints.map(w => `- ${w.title}（当前掌握度 ${w.weightedScore}%，等级：${w.level === 'needs_practice' ? '需练习' : w.level === 'developing' ? '发展中' : w.level === 'proficient' ? '熟练' : '掌握'}，最近更新：${new Date(w.lastUpdated * 1000).toLocaleDateString('zh-CN')}）`).join('\n');
                    }
                    else {
            ...
            const humanMsg = body.images?.length
                ? new HumanMessage({
                    content: [
            ...
            await safeDeliverQuestion(question, send, { subject: body.subject ?? 'math', grade: body.grade }, body.threadId, owned ? body.conversationId : undefined);
            ...
            // 发送每日用量信息（供前端更新剩余次数）
            if (usageInfo)
                await send({ type: 'usage', ...usageInfo });
            // 流成功完成，才递增用量
            if (shouldIncrement)
...
app.post('/api/explore', async (c) => {
    ...
    if (!entry)
        return c.json({ error: 'not_found', message: '该主题暂无探索课程' }, 404);
    ...
    return streamSSE(c, async (stream) => {
        ...
        try {
            ...
            await safeDeliverQuestion(question, send, { subject: body.subject ?? 'math', grade: body.grade }, threadId, threadId);
            await send({ type: 'done' });
        }
        catch (err) {
            try {
...
function extractStudentAnswerText(answer) {
    if (answer.type === 'multiple_choice')
        return answer.selectedKeys?.join(', ') ?? null;
    if (answer.type === 'fill_blank')
        return answer.answers?.join(' | ') ?? null;
...
function autoCollectChatMistake(userId, subject, grade, mode, toolName, toolArgs, answer, result, chatModel, 
/** 答题前的熟练度分数（用于 before_score，未传则读 DB 当前值） */
beforeProficiencyScore, 
/** 缓存模式下的预期熟练度分数（用于 after_score，优先于 DB 值） */
afterExpectedScore) {
    // 仅归集得分率 < 60% 的题目
    if (result.maxScore <= 0 || result.score / result.maxScore >= 0.6)
        return;
    const now = Math.floor(Date.now() / 1000);
    const id = `mistake-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
    const stem = toolArgs.stem ?? toolArgs.question ?? '';
    const knowledgePoint = result.knowledgePoints?.[0];
    ...
    if (answer.type === 'multiple_choice')
        ...
        errorType = '概念混淆';
    else if (answer.type === 'short_answer')
        errorType = '步骤跳步';
    try {
        db.prepare(`
    ...
    `).run(id, userId, subject, grade, (stem.split(/\n|。|\./)[0] ?? '对话错题').slice(0, 32), stem.slice(0, 8000) ?? '', studentAnswerText, correctAnswer, (result.explanation ?? toolArgs.explanation ?? '').slice(0, 2000) || null, errorType, `答题错误（${result.score}/${result.maxScore} 分）`, 0.3, matchScore, 0, now, now);
        ...
        if (result.knowledgePointId && knowledgePoint) {
            const node = db.prepare("SELECT id FROM kg_nodes WHERE id=? AND type='knowledge_point' AND subject=?")
                .get(result.knowledgePointId, subject);
            if (node) {
        ...
        if (chatModel) {
            ...
            chatModel.invoke([new SystemMessage('你是标题生成助手，只输出纯文本，不超过10个字。'), new HumanMessage(`为以下错题生成一个精炼标题（10字以内，概括核心考点）：\n${stemText}`)]).then((res) => {
                const titleText = typeof res.content === 'string' ? res.content.trim().slice(0, 32) : '';
                if (titleText) {
                    db.prepare('UPDATE mistake_items SET title=?, updated_at=? WHERE id=? AND user_id=?').run(titleText, Math.floor(Date.now() / 1000), id, userId);
                }
            }).catch(() => { });
        }
    }
...
app.post('/api/answer', async (c) => {
    ...
    return streamSSE(c, async (stream) => {
        ...
        try {
            ...
            const subject = String(state.values?.subject ?? 'math');
            const grade = String(state.values?.grade ?? '');
            const taxonomy = resolveQuestionTaxonomy({
                subject,
                grade: grade || undefined,
                knowledgePointId: target.args.knowledgePointId,
                knowledgePointTitle: target.args.knowledgePoint,
                allowedKnowledgePointIds: grade ? getPublishedKnowledgePointIds(subject, grade) : [],
            });
            if (!taxonomy)
                throw new Error('题目知识点不属于当前已发布课程，已拒绝写入画像或展示');
            ...
            let beforeProfScore = null;
            if (userId && result.knowledgePointId) {
                const br = db.prepare('SELECT weighted_score FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, result.knowledgePointId);
                if (br)
                    beforeProfScore = br.weighted_score;
            }
            ...
            if (userId) {
                ...
                if (result.knowledgePointId) {
                    const node = db.prepare("SELECT id, title FROM kg_nodes WHERE id=? AND type='knowledge_point' AND subject=?").get(result.knowledgePointId, subject);
                    if (node)
                        nodesToUpdate.push(node);
                }
                for (const node of nodesToUpdate) {
                    const currentMode = state.values?.mode ?? 'qa';
                    ...
                    const qDifficulty = difficultyLevelToValue(target.args?.difficulty);
                    if (isCached) {
                        ...
                        const dbRow = db.prepare('SELECT rating, rating_sigma, last_updated FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, node.id);
                        const { rating: oldRating, sigma: oldSigma, lastUpdated } = getCachedProficiencyExpected(userId, body.threadId, node.id, dbRow?.rating ?? ELO_RATING_INIT, dbRow?.rating_sigma ?? ELO_SIGMA_INIT, dbRow?.last_updated ?? 0);
                ...
                `).all(node.id);
                                for (const pre of prereqRows) {
                                    const preDbRow = db.prepare('SELECT rating, rating_sigma, last_updated FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, pre.id);
                                    cacheProficiencyUpdate(userId, body.threadId, pre.id, 0, 0, currentMode);
                                    const preState = getCachedProficiencyExpected(userId, body.threadId, pre.id, preDbRow?.rating ?? ELO_RATING_INIT, preDbRow?.rating_sigma ?? ELO_SIGMA_INIT, preDbRow?.last_updated ?? 0);
            ...
            if (profChanges.length) {
                result.proficiencyChanges = profChanges;
            }
            // 发送判分结果（含熟练度变化）
            await send({ type: 'grading', toolCallId: body.toolCallId, result });
            // 缓存模式下读取预期熟练度（供错题归集用，比 DB 值更准确）
            let afterExpectedScore = null;
            const isCachedMistake = ['review', 'preview', 'weakness', 'practice', 'explore'].includes(state.values?.mode ?? '');
            if (isCachedMistake && userId && result.knowledgePointId) {
                try {
                    ...
                    if (sum && sum.score > 0) {
                        const dbR = db.prepare('SELECT rating, rating_sigma, last_updated FROM user_kp_proficiency WHERE user_id=? AND kg_node_id=?').get(userId, result.knowledgePointId);
                        ...
                        if (cachedState.rating > 0)
                            afterExpectedScore = cachedState.rating;
                    }
                }
                catch { /* 静默 */ }
            }
            ...
            try {
                const chatSubject = state.values?.subject ?? 'math';
                const chatGrade = String(state.values?.grade ?? '7');
                const chatMode = state.values?.mode ?? 'qa';
            ...
            const { last, question } = await runGraph(new Command({
                resume: {
                    type: 'answer',
            ...
            await safeDeliverQuestion(question, send, { subject, grade: grade || undefined }, body.threadId, body.conversationId || undefined);
            await emitReviewCompleteIfAny(last, send);
            if (answerShouldIncrement)
                incrementUsage(userId);
            await send({ type: 'done' });
        }
        catch (err) {
