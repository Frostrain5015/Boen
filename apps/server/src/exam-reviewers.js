...
import { reviewCorrectnessPrompt, reviewSimilarityPrompt, reviewBlueprintMatchPrompt, reviewFormatPrompt, reviewDiscriminationPrompt, regenerateQuestionPrompt, } from './exam-prompts.js';
// ── 评分权重（可配） ────────────────────────────────────────
...
/** 构造审核 tool（仅作结构化输出契约） */
function makeReviewTool(name, description, schema) {
...
function withTimeout(p, ms, label) {
    return Promise.race([
        p,
        new Promise((_, reject) => setTimeout(() => reject(new Error(`[review] ${label} 超时 (${ms}ms)`)), ms)),
    ]);
}
function failedReviewResult(questionCount, dimension, reason) {
    return {
...
async function runSingleReview(model, prompt, _toolName, schema, dimension, questionCount) {
    // 使用 DeepSeek JSON Output 模式（response_format），prompt 已要求输出 JSON
    try {
        const response = await withTimeout(model.invoke([new SystemMessage(prompt + '\n\n必须直接输出纯净 JSON，不要 markdown 代码块，不要其他文字。')], { response_format: { type: 'json_object' } }), REVIEW_TIMEOUT, dimension);
        ...
        if (parsed.success)
            return parseReviewOutput(parsed.data, dimension, questionCount);
...
function parseReviewOutput(data, dimension, questionCount) {
    ...
    for (const s of rawScores) {
        ...
        byIndex.set(index, {
            ...
            issues: Array.isArray(s?.issues) ? s.issues.map((issue) => String(issue)) : [],
            similarTo: Array.isArray(s?.similarTo) ? s.similarTo.filter((i) => Number.isInteger(i)) : undefined,
        });
    }
    const scores = Array.from({ length: questionCount }, (_, index) => byIndex.get(index) ?? {
        dimension,
 ...
 */
export async function reviewBoard(model, questions, config, blueprint) {
    // 本地格式预检（快速失败，不走 LLM）
    ...
    const scores = questions.map((q, i) => {
        ...
        const total = Object.entries(DIMENSION_WEIGHTS).reduce((sum, [dim, weight]) => {
            return sum + dimScores[dim].score * weight;
        }, 0);
        // 重出决策
...
function detectLocalFormatIssues(questions) {
...
// ── 重出反馈生成 ────────────────────────────────────────────
function buildRegenerationFeedback(dimensions) {
    const lines = ['这道题在审核中得分如下：'];
    const dimLabels = {
        correctness: '正确性',
        similarity: '相似性',
    ...
    const failedDims = Object.entries(dimensions).filter(([, d]) => d.score < 60).map(([dim]) => dim);
 ...
 */
export async function regenerateQuestions(model, questions, scores, config, crossGroupContext, writeSingleQuestion) {
    const regenPool = scores.filter(s => s.needsRegeneration);
    const regeneratedIndices = [];
    ...
    // 并发重出（限 5 路，429 退避重试）
    const regenResults = await withConcurrencyLimit(regenPool.map((s) => async () => {
 ...
 */
function lightValidate(question, allQuestions, selfIndex, failedDims, original) {
    // 基本结构校验
    if (!question.stem || question.stem.length < 5)
        return false;
    ...
    for (const dim of failedDims) {
        switch (dim) {
            case 'format': {
                if (!validateKatexClosure(question.stem))
                    return false;
                if (question.explanation && !validateKatexClosure(question.explanation))
                    return false;
                if (question.type === 'multiple_choice' && (!question.options || question.options.length < 3))
                    return false;
                break;
            }
            case 'similarity': {
            ...
            case 'correctness':
                // 无法本地校验知识正确性，仅校验非空
                if (question.type === 'multiple_choice' && !question.correctKeys?.length)
                    return false;
                if (question.type === 'true_false' && typeof question.answer !== 'boolean')
                    return false;
                if (question.type === 'fill_blank' && !question.blanks?.length)
                    return false;
                if (question.type === 'short_answer' && !question.referenceAnswer)
                    return false;
                break;
            case 'discrimination':
            case 'blueprint_match':
...
/** 校验 KaTeX $ / $$ 成对闭合 */
function validateKatexClosure(text) {
    // $$ 成对
 ...
 */
export async function detectSimilarQuestions(questions) {
...
function detectSimilarQuestionsCharLevel(questions) {
    const flagged = new Set();
