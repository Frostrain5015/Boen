...
/** 按学科获取主题领域 */
export function themesForSubject(subject) {
    switch (subject) {
...
/** 按学科获取核心素养 */
export function literaciesForSubject(subject) {
    switch (subject) {
...
// ── DB 初始化：建表 ──────────────────────────
export function ensureKnowledgeGraphTables() {
...
export function seedKnowledgeGraph() {
    ...
    const insNode = db.prepare(`INSERT OR IGNORE INTO kg_nodes (type, subject, code, title, description, metadata) VALUES (?, ?, ?, ?, ?, ?)`);
    ...
    for (const [themeCode, litCodes] of Object.entries(mathThemeLiteracyMap)) {
        const themeNode = db.prepare(`SELECT id FROM kg_nodes WHERE type='theme' AND code=?`).get(themeCode);
        if (!themeNode)
            continue;
        for (const litCode of litCodes) {
            const litNode = db.prepare(`SELECT id FROM kg_nodes WHERE type='literacy' AND code=?`).get(litCode);
            if (litNode)
                insEdge.run(themeNode.id, litNode.id, 'belongs_to', 1.0);
        }
    }
    // 语文
    const chineseThemeLiteracyMap = {
...
// ── 查询接口 ─────────────────────────────────
/** 获取某类型下的所有节点 */
export function getNodesByType(type, subject) {
    const rows = subject
...
/** 获取某节点的所有邻居 */
export function getNeighbors(nodeId, edgeType) {
    const results = [];
    if (edgeType) {
    ...
    `).all(nodeId, edgeType);
        for (const r of outgoing)
            results.push({ nodeId: r.id, type: r.type, title: r.title, edgeType: r.edge_type, weight: r.weight, direction: 'outgoing' });
    ...
    `).all(nodeId, edgeType);
        for (const r of incoming)
            results.push({ nodeId: r.id, type: r.type, title: r.title, edgeType: r.edge_type, weight: r.weight, direction: 'incoming' });
    }
    else {
        const outgoing = db.prepare(`
      SELECT n.id, n.type, n.title, e.type AS edge_type, e.weight
...
/** 自由映射：将章节与知识图谱节点关联 */
export function mapUnitToKgNode(unitId, nodeId, relevance = 1.0) {
...
export function syncUnitMappings(subject, grade) {
  ...
  `).all(subject, grade);
    ...
    for (const row of rows) {
        ...
        let node = db.prepare(`SELECT id FROM kg_nodes WHERE type='knowledge_point' AND title=? AND subject=?`).get(row.kp_title, subject);
        if (!node) {
...
/** 获取章节关联的知识图谱节点（含素养链） */
export function getKgContextForUnit(unitId) {
...
export function formatKgContext(unitId) {
