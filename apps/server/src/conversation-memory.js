...
function parseSummary(text) {
    ...
    if (sections['摘要'])
        result.summary = sections['摘要'].join(' ');
    if (sections['主题标签'])
        result.topics = sections['主题标签'].join(' ');
    if (sections['学生水平'])
        result.proficiencyLevel = sections['学生水平'][0].trim().toLowerCase();
    if (sections['未解决问题'])
        result.unresolvedQuestions = sections['未解决问题'].join(' ');
    return result;
}
/**
 * 使用 LLM 从对话消息生成结构化摘要。
 */
export async function generateConversationSummary(model, messages, subject) {
    // 只取最后 30 条消息，避免超长
 ...
 * 同时清理该用户超出上限的旧摘要。
 */
export function storeConversationSummary(userId, conversationId, subject, summary, topics, proficiencyLevel, unresolvedQuestions, embedding) {
  ...
  `).run(id, userId, conversationId, subject, summary, topics, proficiencyLevel, unresolvedQuestions, now, vectorToBlob(embedding));
    // 限制条数：超出则删除最旧的
    const count = db.prepare(`SELECT COUNT(*) as cnt FROM conversation_summaries WHERE user_id = ?`).get(userId);
 ...
 */
export async function generateConversationSummaryAsync(userId, conversationId, subject, model) {
    try {
        ...
        storeConversationSummary(userId, conversationId, subject, parsed.summary, parsed.topics, parsed.proficiencyLevel, parsed.unresolvedQuestions, emb);
 ...
 */
export async function retrieveRelevantMemories(userId, query, subject) {
    if (!userId || !query)
        return '';
    // 取该用户最近 30 天的摘要
  ...
  `).all(userId, subject, cutoff);
    ...
    if (!scored.length)
        return '';
    const lines = scored.map(r => `- 主题：${r.topics || '未知'} | 水平：${r.proficiencyLevel}
  摘要：${r.summary}
