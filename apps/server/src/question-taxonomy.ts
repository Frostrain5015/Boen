...
export interface TaxonomyLookup {
  ...
  /** 只用于把 LLM 选中的候选项反查到数据库；不会作为返回展示文本。 */
  knowledgePointTitle?: unknown;
  allowedKnowledgePointIds?: number[];
}
...
export function getQuestionTaxonomyById(
  knowledgePointId: number,
  subject: string,
  grade?: string,
  ...
  `).get(knowledgePointId, subject, grade ?? null, grade ?? null, subject) as { id: number; title: string } | undefined;
  if (!row) return null;
  ...
  `).all(row.id) as Array<{ title: string }>;
 ...
 */
export function resolveQuestionTaxonomy(input: TaxonomyLookup): QuestionTaxonomy | null {
  ...
  if (requestedTitle && allowed.length > 0) {
    for (const id of allowed) {
      ...
      if (taxonomy?.knowledgePoint === requestedTitle) return taxonomy;
    }
  }
...
export function getPublishedKnowledgePointIds(subject: string, grade: string): number[] {
  return (db.prepare(`
    ...
    ORDER BY n.id
  `).all(subject, subject, grade) as Array<{ id: number }>).map((row) => row.id);
}

/** 仅用于不信任历史题目时的展示和结果修复。 */
export function canonicalizeStoredQuestionTaxonomy<T extends {
  knowledgePointId?: number;
  knowledgePoint?: string;
  literacies?: string[];
}>(question: T, subject: string, grade?: string): T | null {
