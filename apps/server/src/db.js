...
`);
// Elo 列迁移
const profColumnsMigration = db.prepare(`PRAGMA table_info(user_kp_proficiency)`).all();
const hasProfCol = (name) => profColumnsMigration.some((col) => col.name === name);
...
const mpeColsMigration = db.prepare(`PRAGMA table_info(mistake_proficiency_events)`).all();
const hasMpeCol = (name) => mpeColsMigration.some((col) => col.name === name);
if (!hasMpeCol('before_rating'))
    db.exec(`ALTER TABLE mistake_proficiency_events ADD COLUMN before_rating REAL`);
if (!hasMpeCol('after_rating'))
    db.exec(`ALTER TABLE mistake_proficiency_events ADD COLUMN after_rating REAL`);
if (!hasMpeCol('before_sigma'))
    db.exec(`ALTER TABLE mistake_proficiency_events ADD COLUMN before_sigma REAL`);
if (!hasMpeCol('after_sigma'))
    db.exec(`ALTER TABLE mistake_proficiency_events ADD COLUMN after_sigma REAL`);
// ── 订阅系统 ─────────────────────────────────────────
db.exec(`
...
{
    const cols = db.prepare(`PRAGMA table_info(mistake_items)`).all();
