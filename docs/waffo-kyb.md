...
- 功能版本 5c5cb4c 已推送 main 并部署到阿里云；发布构建检查、服务端 TypeScript 检查及 128 项自动化测试通过。
- 本地和线上各通过 32 组会员状态／屏幕尺寸检查。会员接口使用测试夹具验证 UI，不代表真实支付已完成；动态卡面、键盘翻转、移动端布局及全站页脚均已检查。
- 线上 /pricing、/terms、/privacy 无需登录可访问；已核实隐私政策中的运营者提供地址。健康检查 200，未登录订阅管理接口 401，无效签名 Webhook 401。
- 部署前 SQLite 在线备份：apps/server/data/backups/boen-20260923T004530Z.db，完整性检查通过；5 条历史会员记录部署前后摘要一致。旧前端保存在 /var/backups/boen/releases/20260923T004530Z/。
- PM2 日志轮换已安装，公开测试结账已关闭。Waffo 实际测试收银台展示 7 天免费试用、今日 USD 0、之后 USD 3/月；尚未提交测试付款，真实沙盒取消／恢复与回调闭环仍待验证。
- KYB 仍为未提交，商品无生产版本。网站自动自检复检、邮箱／域名／退款设置核验、KYB 批准及生产支付闭环尚未完成，不能据本次代码部署宣称通过 KYB。
