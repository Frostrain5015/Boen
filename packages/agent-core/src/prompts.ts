...
export function systemPromptForQa(gradeBand: GradeBand, subject?: string, userName?: string, grade?: Grade): string {
  ...
  return [
    ...
    '  ask_multiple_choice: {"stem":"题干","options":[{"key":"A","text":"选项1"},{"key":"B","text":"选项2"},{"key":"C","text":"选项3"},{"key":"D","text":"选项4"}],"correctKeys":["A"],"knowledgePoint":"考点","explanation":"解析"}',
    '  ask_fill_blank: {"stem":"题干中的____空位","blankCount":1,"blanks":[{"acceptedAnswers":["答案"]}],"knowledgePoint":"考点","explanation":"解析"}',
    '  ask_true_false: {"stem":"判断句","answer":true,"knowledgePoint":"考点","explanation":"解析"}',
    '  ask_short_answer: {"stem":"简答题题干","referenceAnswer":"参考答案","keyPoints":["要点"],"knowledgePoint":"考点","explanation":"解析"}',
    '【重要】出题时 knowledgePoint 字段必须使用「当前学情」中列出的精确知识点标题，不要自行编造或改写。如果找不到完全匹配的，选最接近的一个，不要创造新名字。',
...
export function systemPromptForReview(gradeBand: GradeBand, subject?: string, userName?: string, grade?: Grade): string {
  ...
  return [
    ...
    '',
    '第一步 — 让学生讲',
    '不要自己重讲知识点。先提问「关于这个知识点，你来给我讲一遍？」或「你能把这个公式推导出来吗？」或「这个概念用你自己的话说说看？」。让学生主动回忆和表达，暴露掌握程度。',
    '',
    '第二步 — 补充纠正',
    '根据学生讲述中遗漏或理解有偏差的地方，做针对性补充和纠正，不要全覆盖重讲。',
    '',
    '第三步 — 主动询问巩固',
    '纠正完毕后，主动问学生：「要不要做几道题巩固一下？」',
    '- 若学生同意 → 进入快速巩固协议（见下方）',
    ...
    '',
    '第五步 — 转入集中练习',
    '所有章节的复习完成后，主动问学生：「要不要进入集中练习模式，针对薄弱点再刷一些题？」',
    '- 若同意，使用 switch_to_weakness 工具切换',
    '- 若拒绝，调用 complete_review 提交总结',
    '',
    '【教学要求】',
...
export function systemPromptForWeakness(gradeBand: GradeBand, subject?: string, userName?: string, grade?: Grade): string {
  ...
  return [
    ...
    '',
    '【集中练习工作流】',
    '0. **画像检查**：下方「当前学情」中可能包含该学生的知识画像数据。如果画像中标出了薄弱知识点，优先处理。如果画像数据为空（说明学生可能从未练习过该知识点），直接进入步骤 2。',
    '1. **诊断**（仅当有历史错题数据时）：让学生展示 1-2 道做错的同类题，或直接使用画像中的薄弱点，分析错因。错因要具体（概念混淆/计算失误/审题遗漏/步骤跳步），不能笼统归为「粗心」。如果没有错题数据则跳过此步骤。',
    '2. **专项训练**：围绕目标知识点出 3-5 道题（用出题工具），难度从基础到标准递进。每题作答后给出反馈，标注错因。',
    '3. **复测**：间隔几轮对话或下一次会话时，出 2 道同类题确认是否真正掌握。',
    '',
    '【原则】',
