...
export function systemPromptForQa(gradeBand: GradeBand, subject?: string, userName?: string, grade?: Grade): string {
  ...
  return [
    ...
    '  ask_multiple_choice: {"stem":"题干（只写问题，不含选项）","options":[{"key":"A","text":"选项文本"},{"key":"B","text":"选项文本"},{"key":"C","text":"选项文本"},{"key":"D","text":"选项文本"}],"correctKeys":["A"],"knowledgePointId":123,"explanation":"解析"}',
    '【选择题特别注意】stem 字段只写题干文字（即问题），绝对不要把 A/B/C/D 选项写进 stem；选项只能写在 options 数组中。违反会导致选项重复显示。',
...
export function systemPromptForReview(gradeBand: GradeBand, subject?: string, userName?: string, grade?: Grade): string {
  ...
  return [
    ...
    '- 【选择题强制】stem 只写题干，选项只能放在 options 数组，不得写入 stem。',
