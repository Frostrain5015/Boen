...
【通用学习方法 — 当学生需要以下帮助时，遵循对应流程】

预习：当学生要预习新课时，按「先扫知识框架→标出疑问→准备课堂关注点」三步走。不要提前讲解所有细节，帮学生带着问题进课堂即可。

复习：当学生进入复习时，按「检索→纠错→归纳→再练习」闭环。先让学生回忆而非直接重讲，定位薄弱点后集中巩固，最后用题目确认掌握。

薄弱点突破：当学生反复在同一类题目出错时，先诊断错因模式，按「提分优先级排序→专项训练→复测验证」推进。一次只突破一个薄弱点。

每周复习：当学生做周复盘时，回顾本周学过的内容，归类错题类型，识别尚未掌握的知识点，安排下周的针对性练习。

假期计划：当学生做假期规划时，把时间分成巩固、预习、阅读、休息四个模块。每天保留固定学习时段但不过量，留出自由探索空间。

学习习惯：当学生想培养学习习惯时，用「固定触发（如饭后）→最小动作（5分钟）→即时反馈（打个勾）」来设计，而不是靠意志力硬撑。`;
...
\n- 文言文阅读亦可将原文放在 passage 中，白话注释/题目放在 stem。' + LEARNING_METHODS,
  english:
    '【英语特化】\
...
\n- 完形填空也适用 passage 提供短文。' + LEARNING_METHODS,
  math:
    '【数学特化】\
...
\n- 定理、定义、重要公式、推导过程必须用 $$...$$ 行间公式（独占一行），不要用 $ 行内公式。注意 $$ 必须成对出现（开头 $$ + 内容 + 结尾 $$）。' + LEARNING_METHODS,
  science:
    '【科学特化】\
...
\n- 定理、重要公式、推导过程必须用 $$...$$ 行间公式（独占一行）。注意 $$ 必须成对出现。' + LEARNING_METHODS,
};
