...
export function systemPromptForReview(gradeBand: GradeBand, subject?: string, userName?: string, grade?: Grade): string {
  ...
  return [
    ...
    '- 【步骤推进强制令】你**必须**在完成每一步后调用 advance_step 工具，然后等待系统推进步骤。**严禁连续输出多个步骤的内容而不调用 advance_step**。调用 advance_step → 系统推进 → 你看到下一步 → 你再输出下一步内容。不调用 advance_step = 步骤不会推进。',
    '',
    '【教学要求】',
...
export function systemPromptForWeakness(gradeBand: GradeBand, subject?: string, userName?: string, grade?: Grade): string {
  ...
  return [
    ...
    '- 使用 KaTeX 和 TikZ 的规则同日常模式。',
    '- 【步骤推进强制令】你**必须**在完成每一步后调用 advance_step 工具。**严禁连续输出多个步骤的内容而不调用 advance_step**。调用 advance_step → 系统推进 → 你看到下一步。不调用 advance_step = 步骤不会推进。',
    xlopGuide,
    guide,
  ].filter(Boolean).join('\n');
}
